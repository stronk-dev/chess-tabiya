...
test("library exposes phase honestly and survives a malformed pack response", async ({
  ...
  await card.getByRole("button", { name: /Rehearse this position/ }).click();
  await expect(page.getByRole("alert")).toHaveText("Tabiya could not complete that action right now. Reopen the run to check its latest position, then try again.");
