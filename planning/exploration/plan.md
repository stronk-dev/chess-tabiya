...
| Q3 | Mobile: scope or non-goal? | 📊 evidence updated 2026-08-21 — **tolerate, responsive-only remains the scope; the pointer/touch run-loop floor is repaired.** At 390×844 A2 moved from 0/18 to **18/18 exact** click/drag/touch cells and every authored source is hit-testable after D573. The earlier 720–992 px full-surface gap and R18 keyboard/assistive/physical-device work remain; mobile-native is still a non-goal. `design/research/mobile-scope.md`, `design/research/interaction-state-correctness.md` | — |
...
| Q4c | Can automatic phase/structure recognition support Just Play without pretending certainty? | 📊 partial 2026-09-01 — R1 settles the producer boundary; R2 settles selection; R3 proves board lighting and the old LLM packet bypass those gates. D2483 measures the shipped rules convention; D2484 resolves its five typed decision arms and exact threshold sensitivity. D2485 resolves downstream composition: 804 exact-node source vectors and 100 paths prove opening membership overlaps unclear/middlegame rules readings, exits 49 times and re-enters 14, so opening, rules phase, endgame and tablebase remain independently attributed source slots plus source-local history—not one “true phase.” D2495–D2497 split named endgame evidence into material, cited setup, observed path stage, perspective outcome and completeness-bearing quantified proof. Three complete one-ply roots prove Lucena possible/not-forceable, Philidor forceable/not-inevitable and Vančura inevitable under exact WDL preservation, while unexpanded future branches abstain. This is still not eventual reachability, calibrated confidence or independent position truth. Semantic family admission, position labels/error costs, a production multi-ply provider, valid touch/keyboard interaction and nontechnical reader/preset validation remain open | B2, B4 |
...
| Q8 | Can feedback beat "Stockfish labels + prose"? | 📊 split verdict — **YES on honesty, timing and re-entry**; **not yet on specificity or presentation**. The committed baseline delivered none of its authored claim layer; current Stage-1 feedback work is uncommitted. The 2026-08-20 alignment audit adds that the LLM only rewrites selected sentences and the story/review surface lacks semantic, theory and re-entry modules. `design/research/feedback-versus-the-dashboard.md`, `design/research/integrated-platform-alignment.md` | K6, C1 |
