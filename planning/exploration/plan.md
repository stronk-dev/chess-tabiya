...
| Q4c | Can automatic phase/structure recognition support Just Play without pretending certainty? | 📊 partial 2026-08-20 — R1 settles the producer boundary; R2 settles selection; R3's mechanical/desk arm now proves both board lighting and the LLM packet bypass those gates. One occupied-square query reaches 11 captions/19 marks, while settings expose 54 source/mechanism controls. An eight-module boundary passes synthetic leakage/abstention tests, but semantic family admission, valid touch/keyboard interaction and nontechnical reader/preset validation remain open | B2, B4 |
