...
| Q3 | Mobile: scope or non-goal? | 📊 evidence 2026-08-20 — **tolerate, responsive-only remains the scope; the run-loop floor is now measured unmet.** A2's exact-UCI census delivered 0/6 authored moves by click, drag or emulated touch at 390×844; five authored source squares were covered by timeline/control regions before selection. The earlier 720–992 px full-surface gap also remains. Mobile-native is still a non-goal; responsive web needs D573 plus R18 real-device/accessibility evidence. `design/research/mobile-scope.md`, `design/research/interaction-state-correctness.md` | — |
...
| Q4c | Can automatic phase/structure recognition support Just Play without pretending certainty? | 📊 partial 2026-08-20 — R1 settles the producer boundary; R2 `selection-sign-and-significance.md` settles the mechanical delivery boundary. A local ≤20%-alternative-share, cap-two selector cuts raw volume ~11× and reaches 93%+ counterfactual specificity on authored and imported games while retaining 108/108 rare rules events. It also elevates trivial literals, so semantic eligibility must precede selection and relation sign cannot supply valence. Presentation (R3) and reader/per-consumer validation remain open | B2, B4 |
