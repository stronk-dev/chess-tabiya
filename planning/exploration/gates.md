...
| K9 | Endgame mode is not materially faster or more usable than Chess Endgame Training | **📊 EVIDENCE TOWARD FIRING — on the USABILITY clause, 2026-08-16** | `design/research/endgame-latency-versus-cet.md` `[V]`. **The criterion's own-side arm had never been measured** ([[D488]]); it has now. **Speed: every budget met comfortably** — restart 37.9 ms median perceived, rewind 11.9 ms, opponent reply 128.0 ms, `perfect_tablebase` first probe 33.0 ms against real Syzygy. **And K9 cannot be CLEARED on speed either**: measured back-to-back from one machine on the same FENs, CET's endpoint is **30.8 ms** and ours **30.1 ms** — the 80–224 ms in `teardown-cet.md` was that network on that day, not a slower architecture, so **there is no speed gap to win on the shared call**. **Usability: at 1440×1000, 5 of 6 served endgame packs cannot receive their own authored first move; at 1280×720 and 1366×768 all 64 squares of all 6 are unhittable.** The board overflows `.position-column` by 64–164 px and is drawn under the timeline; `.drill-region` does not scroll and `scrollIntoView` moves it 0 px. **Trigger is authored objective length** — schema example 68 chars, endgame packs 277–444 — which is why the one pack production serves is 0/64 occluded and the corpus is not. **Recorded as evidence toward firing rather than `fired`: the cause is a fixable layout defect at a named place, and calling the criterion is the owner's.** The half a stopwatch cannot answer remains open: whether it *feels* immediate, and *"more usable"* in the comparative sense. |
