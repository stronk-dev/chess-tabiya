...
| B3 | Review: manual multi-branch selection, pair/multi compare, replay, deep mode, share/export, **plus branch groups — N candidates forked and played in parallel with resistance held constant** | ✅ met — `n-way-comparison` ships N-way compare, simulate, prediction rendering, deep analysis, and branch-selective export; `branch-groups` ships durable candidate sets, fixed/per-branch resistance, sequential/lockstep play, semantic zoom, evidence recovery, compare, and export |
