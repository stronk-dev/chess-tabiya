...
| K9 | Endgame mode is not materially faster or more usable than Chess Endgame Training | **📊 EVIDENCE TOWARD FIRING — status unchanged, re-measured 2026-08-20** | `design/research/endgame-latency-versus-cet.md` §10 and `design/research/interaction-state-correctness.md` `[V]`. **Speed remains settled and cannot clear K9:** the products use the same tablebase call (30.1 ms ours vs 30.8 ms CET back-to-back), while our restart/rewind/reply budgets pass. **Usability remains below the floor on a repaired instrument:** all six served endgame packs × five viewports × click/drag/emulated touch, exact request UCI rather than “some ply happened.” The authored legal UCI was delivered in **4/90** live cells; **15/90 submitted a different legal UCI** and 71/90 submitted nothing. Live click is **1/6** exact at 1440×1000 and **0/6** at 1366×768, 1280×720, 768×1024 and 390×844. From tablet upward, selection shifts the board **17–96 px** while chessground retains stale bounds; resize recovers **24/24** desktop/tablet cells. At 390×844, **5/6 authored source squares are covered before selection** and resize recovers only the sixth ([[D573]]). The stale-coordinate arm succeeds 19/30 while live click succeeds 1/30, explicitly reproducing rather than cancelling [[D539]]. **Not `open`:** both routes to clearing are absent. **Not `fired`:** the causes remain named implementation defects rather than evidence against the rehearsal idea, and calling the criterion is the owner's. |
...
| B4 | Evidence: authored, Stockfish, Maia, corpus, Syzygy, features, LLM-rendered layers with timing controls | authored ✓, Stockfish ✓, Maia ✓, **corpus/recency ✓ (`runtime-corpus-evidence`)**, structural mechanics ✓ (B9), voice seam ✓. **Feedback Stage 1 (2026-08-21):** the authored-claim delivery path is executable and its three populations are measured rather than conflated: **98/196 evidence-admitted, 98/196 evidence-withheld; after 50/50 full-spine walkthroughs, 69/98 admitted claims render and 29/98 remain timing-withheld without a released reveal occurrence**. **R1–R5/A3–A4 qualification:** mechanics are not one semantic evidence plane. A3 finds only 11/18 structural matcher/reader families round-trip, all six transition families lossy and 0/3,371 transition observations retaining squares. A4 traces 14 producer paths into five incompatible delivery states: only four renderer-visible; runtime/sourcing names have 0 joins; production has 0/9 module and 0/6 workflow IDs; transition/Maia/Explorer/opening identity are absent from `EvidencePacket`, whose sentence allow-list also omits generic observations/plans. R3's module boundary is mechanically viable but reader validation remains external. R5 establishes deterministic rendering as normative and permits only a conformance-gated optional LLM after selection. Residual: the Stage-2 binding wave + F1 compiled projections/consumer dispositions + F2 operand-preserving eligible/selected events + module-bound reader validation |
| B5 | Live: Twitch host/chat/overlay, academy roles, external Arena handoff | mechanically shipped 2026-08-13 (`live-session-platform`) — roles, board control, spectate, chat voting, academy, Arena two-leg handoff. **A5 qualification:** Academy has no assistance profile and inherits its source run; this is not a proven coach workflow or default. Native matchmaking stays outside minimal-real scope by design |
...
| B8 | Platform: desktop shell, responsive/PWA, self-hosted engines/providers, share links, accessibility | **mechanically shipped, release-qualified by R18.** The light profile completes/persists a rehearsal without cloud secrets and share links ship. The 1.0 platform floor is not met: no account export/backup contract; deletion retains solo runs; Maia loss leaves green capabilities and hangs an uncached request; board move entry is pointer-only and incorrect under A2; unmodified Tab traps focus; distributed notices/SBOM are absent; the 5.11 GB Maia image carries proprietary-labelled CUDA dependencies. PWA remains manifest-only. `design/research/release-platform-audit.md` `[V]`; O13/F12 own the release floor. |
