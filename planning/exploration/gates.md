...
| K9 | Endgame mode is not materially faster or more usable than Chess Endgame Training | **📊 EVIDENCE TOWARD FIRING — status unchanged 2026-08-17, reason ENTIRELY REWRITTEN** | `design/research/endgame-latency-versus-cet.md` §10 `[V]`. **Every specific number in the previous cell is now false** — the 2026-08-16 layout defect is **fixed and completely so**: 0 of 64 squares occluded at all five viewports on all six packs, column overflow 64–164 px → **0 px**, and **no length ceiling to 4,000 characters** ([[D540]]) because the fix is a bounded box rather than a tolerance. **The criterion still does not clear, for a different reason** ([[D537]]): selecting a piece shifts the board **17–89 px** and **chessground's bounds cache is never invalidated**, so the authored first move is delivered on **1 of 6** packs at 1440×1000 and **0 of 6** below that — and two packs deliver **a different legal move** than the square clicked (`Rh7+`/`Rh8` for `Rh6`). **Pre-existing and independent of the layout defect.** **Not `open`**: the floor is unmet on measurement and [[D508]] removed the speed route to clearing it — both products call the same tablebase at 30.1 ms against 30.8 ms. **Not `fired`**: the cause is again a defect at a named place, and calling a kill criterion is the owner's. **Warning carried forward from [[D539]]**: session 1's *"1 of 6 playable"* was its probe aiming where the stale bounds decode — probe and bug cancelled — so treat any playability figure here as instrument-sensitive. |
