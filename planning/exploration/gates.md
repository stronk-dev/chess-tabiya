...
| B4 | Evidence: authored, Stockfish, Maia, corpus, Syzygy, features, LLM-rendered layers with timing controls | authored ✓, Stockfish ✓, Maia ✓, **corpus/recency ✓ (`runtime-corpus-evidence`)**, structural mechanics ✓ (B9), voice seam ✓. **R1–R5/A3 qualification:** mechanics are not semantic guidance; cheap semantic labels fail external disagreement, local selection promotes trivial literals without prior eligibility, and current board/LLM surfaces bypass selection. A3 finds only 11/18 structural matcher/reader families round-trip, all six transition families lossy and 0/3,371 transition observations retaining squares; five sinks consume whole readings. R3's module boundary is mechanically viable but reader validation remains external. R5 establishes deterministic rendering as the normative fallback and permits only a conformance-gated optional LLM after selection; schema/IDs alone do not preserve citations/disclosure. Residual: Syzygy runtime rendering + operand-preserving eligible/selected semantic evidence + module-bound reader validation |
