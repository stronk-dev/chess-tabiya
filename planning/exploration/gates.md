...
| B2 | Solo modes: Just Play + Line/Plan/Outcome/Trajectory each complete one fixture run | shipped 2026-08-13 for the four modes (`line-drill-theory-grading`, `outcome-drill-grading`, `trajectory-drill`, Plan) with fixture/browser runs. **Just Play corrected 2026-08-14: F2 shipped the *server* machinery only — the client refuses position runs (`session-controller.ts:180-185`, "the position player is not built yet"), so the B2 row overclaimed for a day.** The minimal position player is scoped inside `rfc/shape-library.md` |
| B3 | Review: manual multi-branch selection, pair/multi compare, replay, deep mode, share/export, **plus branch groups — N candidates forked and played in parallel with resistance held constant** | largely shipped (`n-way-comparison`) — N-way compare, simulate, prediction rendering, deep analysis, branch-selective export. Open surface: **branch groups** (owner 2026-08-13, no RFC yet) |
| B4 | Evidence: authored, Stockfish, Maia, corpus, Syzygy, features, LLM-rendered layers with timing controls | authored prose, Stockfish grounds and Maia policy render; **structural layer is B9** (`rfc/structural-reading.md`, draft); corpus/Syzygy runtime rendering and evidence-bound LLM rendering remain unmet |
...
| B6 | Create: pack studio/import/review/session-distill produces a validated fixture; corpus mining emits one candidate | shipped — mining (`candidate-emit`) plus studio write path, session distillation, imports and publication channels (`pack-studio`) |
| B7 | Return: history/resume, progress, concept scheduling, related retry, recommendations | shipped 2026-08-13 (`return-and-progression`) — attempt scheduling, progress, `/learn`, duplicate, related retry, opt-in recommender. Cross-pack concept identity deliberately absent (a studio/B11 contract) |
