...
| B3 | Review: manual multi-branch selection, pair/multi compare, replay, deep mode, share/export, **plus branch groups — N candidates forked and played in parallel with resistance held constant** | pairwise + grounded objective/evidence rendering. **Pairwise is a runtime type constraint, not a UI limit** (`compare.ts:49-64`), and N-way cannot be composed from pairwise calls without aligning unrelated plies. Multi-branch, replay, deep mode, share unmet |
