...
| D1736 | foundation source-repair authoring wave → Claude/RFC register owner | Consume `foundation-source-repair-wave-2026-08-26.md`: declare shared-resource ownership and landing order once; source identity precedes operations, validation, avoidance and module migration. Every semantic handoff must be referenced exactly once. |
...
| D2032, D2033, D2034 | `provider-exchange-and-execution` fourth-return author amendment | Separate Syzygy's local no-exchange domain fact from receipt-bearing provider success; give subject availability a bounded ownership/role/operation access matrix; and make compiled source requirements occurrence- plus exact-request-addressed before any deduplication. Replace the three review reproductions with crossed author fixtures, then fresh independent review before implementation. |
