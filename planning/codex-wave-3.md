...
## Graduation-clearance migration boundary — [[D3071]], 2026-09-07

The accepted `graduation-clearance` implementation is the sequential schema predecessor to pack
population provenance and the first official-content path. Its shipped read-only planner still
enforces the superseded D560 boundary even though [[D3033]] licenses foundation/schema migrations.
Build a total, typed migration proposal first: every one of the 436 graduation entries must carry a
proposed v0.28 disposition, every proposed field must name whether it was derived or requires an
author, and the tool must refuse unknown populations rather than default them. Only then can the
atomic schema/corpus/restamp slice be priced honestly; authored chess truth and claim-binding work
remain held.

The proposal returned—not papered over—[[D3072]]: only three of nine emitter templates are ready;
six templates covering 104 candidate entries are contract-blocked by missing or mismatched
predicates. An RFC amendment must make those predicates representable before schema 0.28 can be
applied.

Checkpoint: [[D3071]] is complete. `planning/archive/graduation-clearance/migration-proposal.json` covers
436/436 entries and measures **100 ready / 232 author-required / 104 contract-blocked**. The
committed artifact is regenerated by `make graduation-plan-update` and byte-checked by
`make graduation-plan-check`; changed corpus populations, duplicate entry addresses, invalid field
provenance and emitter-registry drift all refuse the check.

## CI parity repair — [[D3076]]/[[D3077]], 2026-09-07

The latest pushed pair at `78be38f0` failed for two locally diagnosable reasons. Repository
governance runs a retained historical-review fixture via `git show` while its GitHub job checks out
only depth 1; that job must request the history its contract consumes, with scaffold coverage at the
job boundary. The import/re-entry browser journey waits on a branch/node census already true before
