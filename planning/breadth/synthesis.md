...
| D2 | **The withholding barrier fails open.** *(Corrected 2026-08-12: there are **six** withholding surfaces, not five — `authoredFeedback` (`service.ts:335`) also branches on the registry and fails **closed**. It did not exist when this pass ran; the F1 implementation added it and shifted line numbers in this file by ~4.)* `publicNodes` and `publicEvents` return everything when `pack === undefined` (`feedback-policy.ts:21,48`). The moment F2 lands, ADR-0006 is violated silently. `service.ts:186,205` also enqueue no evidence for a pack-less run, so there would be nothing to withhold either — both halves must flip in the same slice | verified | latent, becomes live with F2 |
