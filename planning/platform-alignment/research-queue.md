...
| ID | Status | Research deliverable | Method and population | Exit criterion | Unlocks |
|---|---|---|---|---|---|
| R7 | **CURRENT-PRODUCT + TARGETED DESK ARMS DONE 2026-08-21; fixed-game candidate and owner use remain** | **Grounded Review Map and social recap** — `design/research/review-map-and-reentry.md` | Shipped Story has preserved rewind→fork re-entry and deterministic evidence, but 0 F2 semantic inputs, one moment action, divergent private/public top-eight policies and a false engine-only card footer. Current sources separate Chess.com ritual, Lichess verdict-hidden retry, Beacon reply continuity and Quackmate one-moment/share pull from their refused engine-grade/causal-prose forms. | Next: run current/eval-only/mixed-exact policies over the same fixed games; every retained moment needs an action and one shared private/public identity. | O7 choice set and F6 gate. |
