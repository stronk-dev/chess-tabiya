...
| R3 | **MECHANICAL/DESK/RESPONSIVE PROTOTYPE DONE; external participant arm DESCOPED by D649; owner-use validation remains** | **Evidence presentation** — `design/research/evidence-presentation.md`; `planning/platform-alignment/evidence-presentation/participant-plan.md` | Source/corpus audit measured 54 primary controls and a one-square tail of 11 captions/19 marks; the board and LLM paths both bypass R2. Nine modules, five presets and six workflow ceilings pass 17 synthetic tests; the responsive artifact exercises Support, theory-only, honest empty/unavailable and advanced dispositions. D617–D619 settle the architecture/pre-commit boundary. D537/D573 are repaired at 90/90 exact live gestures, so the shipped board is now a valid pointer/touch baseline. | No recruited panel remains. Candidates are validated by the owner's use after real compiler-admitted packets exist; keyboard/assistive scope remains F12/R18. | Exact preset/module defaults; assistance RFC gate. |
