...
| Guided play and blunder prevention | **mechanically present** | `assistance.ts`, `pivotal.ts`, feedback policy, guard conditions, reveal route | Assistance settings/grid, board lighting/arrows, post-commit guard and markers | `immediate-guard.browser.json` plus served draft packs | Browser immediate-guard/passive-guidance episodes; A2 + permanent exact-UCI gate | Defaults are silent; source-shaped settings dominate; the assistance RFC records unreachable/unbound rungs. A2 interaction is repaired at 90/90 exact live gestures, but no validated intent preset/module composition exists. |
...
| Responsive/accessibility/PWA | **mechanically present; pointer/touch correct, assistive interaction inaccessible** | Browser-independent runtime; static web manifest | Responsive shell, keyboard routes/help and labelled controls; board is one generic non-focusable node | Real served draft layouts including six endgames | A2 exact-UCI gesture census plus R18 Chromium DOM/AX/keyboard/viewport probe | A2 repair: 90/90 exact click/drag/touch cells and phone 18/18. R18: no keyboard/assistive move entry and Tab traps at Assistance. Owner device/screen-reader proof remains; manifest exists and service-worker registrations remain zero. |
...
2. R3's disposable prototype/mechanical arms are complete against A2's exact-UCI interaction
   contract. D537/D573 now pass 90/90 live gestures, so the shipped pointer/touch board is a valid
   baseline; D649 descopes the recruited-participant arm and leaves owner-use/default decisions.
