...
| Responsive/accessibility/PWA | **mechanically present** | Browser-independent runtime; static web manifest | Responsive shell, mobile region tabs, keyboard routes/help, ARIA-labelled board and controls | Real served draft layouts including six endgames | Resting projections plus A2 click/drag/touch/hover/resize census at five viewports | A2 refutes gesture correctness: at 390×844 five of six authored source squares are covered and all three live input paths deliver 0/6 exact moves. No screen-reader/real-device floor. Manifest exists but the browser test explicitly proves zero service-worker registrations, so installability is partial, not a complete PWA. |
