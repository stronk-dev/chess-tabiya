...
| Self-hosted/open distribution | **mechanically present** | AGPL repo, server Dockerfile, Compose mock default and optional Maia profile, SQLite volume, release template | Web app reports provider availability | Core app runs with mock profile; engines are optional | Packaging/scaffold verification in A0; no fresh A1 install/upgrade/backup exercise | Build/deploy mechanics exist, but provider-off usability, rights register, export/delete completeness, backup/restore, upgrade and resource-budget proof remain R18/F12 work. |
| Responsive/accessibility/PWA | **mechanically present** | Browser-independent runtime; static web manifest | Responsive shell, mobile region tabs, keyboard routes/help, ARIA-labelled board and controls | Real served draft layouts including six endgames | Browser desktop/tablet/mobile/manifest and keyboard episodes pass at rest | D537–D540 show post-gesture board correctness is not proven; no screen-reader/real-device floor. Manifest exists but the browser test explicitly proves zero service-worker registrations, so installability is partial, not a complete PWA. |
...
| Capability watch | **claimed-only** | Static research matrices only | None/query mechanism absent | Dated competitor dossiers, not a maintained capability record | No watch instrument proof | D556 describes the right process, but there is no capability-first register with canonical identity, loved/hated evidence, dependencies and disposition. |
...
That still does not clear interaction-state defects. The K9 follow-up demonstrated that a resting
viewport invariant could pass after every relevant fixture was added while the board became wrong
after selection/touch. A2 must repair that instrument before any guided-board or mobile result is
promoted from mechanics to UX proof.
...
2. Keep R3 behind A2: the shipped guidance mechanics do not make the interaction instrument honest.
