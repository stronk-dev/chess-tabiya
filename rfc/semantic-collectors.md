...
| D2 | The engine-Review lane: matrix rows `derived.review.eval_delta@1` / `derived.review.mate_transition@1` (typed C4 contract, stage-0 §12.2), the Story mate-type repair (D917, `story.ts:33/:104`) and the multi-source post-game compiler (D918) — codex's order items 3–4, cited here and absorbed by nothing in this RFC; `mate_transition` joins §3.6's proof by node/candidate identity when it lands | `planning/evidence-foundation-ux/plan.md` | the Review-successor RFC's drafting/landing commits | discharged 2026-09-24 by `rfc/review-evidence-compiler.md` implementation: typed `derived.review.eval_delta@1` / `derived.review.mate_transition@1`, the Story mate-type repair and the multi-source packet; `mate_transition` links to `forced_mate_after_move@2` only through the exact `run.record.edge@1` (receipt `planning/evidence-foundation-ux/review-evidence-compiler-implementation-2026-09-24.md`) |
...
The seventh author repair closes all five returns at the bounded contract tier. The result algebra
now retains only exact provider results/invocations or the exact sealed geometry completion, with
parsed source values copied and frozen before admission. Legal maps and pawn contacts are computed
by their shipped FEN-derived authorities; generic same-id declared values containing dropped moves
or invented geometry fail. Recorded/live outcome values cross the shipped tablebase parser and all
ten production categories are executable. `PromotionRaceTablebaseRequest` now carries geometry,
provider scope and abort signal exactly as declared above, while the typed Syzygy invocation keeps
the complete request, result and delivery. The one output is literal
`derived.pawn@1 / derived.pawn.promotion_race_tablebase@1` declared evidence, and its value receipt
retains the exact request, geometry, legal map, selected source and output. `make
semantic-collectors-promotion-seventh-author-repair` retains every prior author/review gate, passes
...
The same-day eighth author repair closes all eight returns at the bounded contract tier. One total
asynchronous operation owns the exact recorded-first/provider/legal order and exposes typed
completed, unavailable and input-abstained results. Its sealed scheduler computes the normalized
request digest; recorded evidence can originate only in the sealed ledger authority; live evidence
can originate only in the exact source factory; and every provider move is checked against the
request FEN before evidence exists. Local-domain, cancellation and failure results cannot cross
requests, and outside-domain evidence reproduces its own piece count. `make
semantic-collectors-promotion-eighth-author-repair` retains every prior promotion gate, passes 8/8
...
The same-day tenth fresh independent review returns that repair on [[D2765]]–[[D2770]]. Its source
snapshot requires a made-up `tabiya.source-manifest.v1`/`sources[]` dialect while production parses
`tabiya.sourcing.manifest.v1`/`entries[]`; the snapshot itself is minted from caller JSON without a
durable read or installed-artifact receipt. Future timestamps and unresolved support pointers pass,
the source registry is self-declared beside the record, and a cloned request reaches the early
outside-domain result before request identity is checked. Public local factories also mint trusted
storage failure and legal unavailability without either dependency operation. `make
semantic-collectors-promotion-tenth-fresh-review` retains the full predecessor chain and passes 6/6
fresh falsifiers. A bounded tenth author repair must consume the production manifest and durable
...
The same-day fourteenth fresh independent review returns the held pair on [[D2892]]–[[D2896]].
`openPromotionApplication(inventoryPath)` remains an exported raw-path authority issuer available
to any importer, and its product path calls the predecessor's explicitly test-only issuer. A
registry can be published while its sole declared generation is missing, because complete
generation validation is deferred until selection. Repeated or concurrent `openRegistry()` calls
mint distinct simultaneously valid current registries, while concurrent first `openGeneration()`
calls cross awaited I/O before populating the store cache and can mint distinct current stores.

`make semantic-collectors-promotion-fourteenth-fresh-review` retains every predecessor return and
repair, then passes 5/5 fresh falsifiers plus strict TypeScript. Exact receipt:
...
The next repair must bind application creation to an exact product composition/configuration
authority, remove the test helper from the product dependency graph, validate the complete listed
generation population before installed-registry publication, and single-flight one registry/store
...
| [[D2650]] | author-repaired: every total-result arm retains exact asserted provider/completion values; arbitrary object arms are deleted |
| [[D2651]] | author-repaired: FEN-derived central legal/contact authorities reject caller arrays and generic same-id values |
...
- 2026-09-24 — Discharge D2 recorded as discharged by the `rfc/review-evidence-compiler.md` implementation (typed eval-delta/mate-transition rows, Story mate-type repair, multi-source packet).
...
- 2026-09-06: fourteenth fresh independent review returned the held promotion pair on
  ...
  repeated registry opens and concurrent first generation opens split current authority. `make
  semantic-collectors-promotion-fourteenth-fresh-review` retains the complete chain and passes 5/5
  fresh falsifiers plus strict TypeScript. Exact receipt:
  `planning/evidence-foundation-ux/semantic-collectors-promotion-fourteenth-fresh-independent-buildability-review-2026-09-06.md`.
  One bounded author repair, another fresh review and provider/value dependency landing remain
  mandatory; the held pair stays unimplemented.
...
- 2026-09-05: tenth fresh independent review returned the ninth held-promotion repair on
  ...
  early trusted result; and public local factories mint dependency failure/unavailability. `make
  semantic-collectors-promotion-tenth-fresh-review` retains the full chain and passes 6/6 fresh
  ...
  The evidence spine remains 12/14 pending a bounded repair, another fresh review and dependency
  landing; no held production id is authorized.
...
- 2026-09-05: eighth author repair closes [[D2693]]–[[D2700]] at RFC tier. One total asynchronous
  ...
  and reachable input abstention. `make semantic-collectors-promotion-eighth-author-repair` retains
  every predecessor gate, passes 8/8 new behavioral groups and strict TypeScript. Exact receipt:
  `planning/evidence-foundation-ux/semantic-collectors-promotion-eighth-author-repair-2026-09-05.md`.
  Ninth fresh review and provider/value dependency landing remain mandatory; no held production id
  is authorized.
...
- 2026-09-04: seventh author repair closes [[D2650]]–[[D2654]] at RFC tier. Exact sealed result
  ...
  five shadow-model seams. `make semantic-collectors-promotion-seventh-author-repair` retains all
  earlier gates, passes 5/5 new behavioral groups and strict TypeScript. Eighth fresh review remains
  mandatory; no held production id is authorized.
