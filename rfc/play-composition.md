...
| D3 | The [[D839]]/[[D840]] lane — dark/board/piece theming over this composition's tokens and the animation pref with a None option; this RFC's A9 removes the animation *defeat* but ships no pref and no theme | `claude` | that lane's landing commit | **2026-08-23 — implemented by `theming.md`; D840's owner felt pass remains that RFC's D5** |
...
- 2026-09-24 — **Module-seat checkpoint.** Receipt: `planning/platform-alignment/evidence-presentation/checkpoint-b-implementation-2026-09-24.md`. §4 seats are live: `ModuleSeats.svelte`
  seats exactly the modules the finalized style composed at a play timing; the collapsed row badge is the
  delivered fact count and on-request rows carry none before a request; one seat expanded; the
  `blunder_prevention` head slot appears only while a staged move is held; §4.3 silent modules render no
  row when empty and stated-absence modules render their sentence inside their card; §4.5 board paint is
  the expanded seat's own facts. The matrix adds states 3, 5, 9 and 13 (A2/A5/A6 over real emitters, with
  a real move submission after state 3). State 6 is not claimed: the Guided Hint seat is the
  hint-distance lane's.
