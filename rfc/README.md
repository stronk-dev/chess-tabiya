...
| `runtime-opening-identity.md` | **draft 2026-08-23 — executes Semantic Collectors D3 from the completed D894 measurement.** Compiles the already-pinned CC0 Lichess catalogue into a deterministic local artifact and keeps exact current named endpoint, current path membership without a guessed descendant, and retrospective deepest reached separate. No sticky label, “out of book,” book policy, theory prose, LLM/FTS applicability or longitudinal claim. Independent buildability review required | `planning/evidence-foundation-ux/` C3/F4/F7 handoff | **claims nothing versioned** — private server artifact; three opening projections plus the narrow recorded-position input |
...
| `exact-legal-mobility.md` | **draft 2026-08-23 — executes D904 from completed D783 research.** Splits exact actual-turn legality from the convention-grounded B/N/R/Q safe-mobility payload; retains all six roles, standard castling UCI, en-passant and four promotion identities; unifies evidence, board input and server successor walking on one runtime authority. No safety, quality, recommendation or preset is inferred | `breadth-collectors.md` §3.2; F1; accessible board input | **claims nothing versioned** — one additive `rules.mobility` projection and an exported runtime helper, no schema/migration lane |
