...
| `board-annotation.md` | **accepted 2026-08-16** — returned once, all four breaks ratified | — | Implements the owner's arrows ruling, legs **(a)** learner-drawn and **(b)** host-relayed; leg (c) stays with `format-surface`. Marks live in a new `run_marks` table, never the run event log. **Claims a migration POSITION, not an integer** — behind `teacher-surface`; whichever lands first takes `STORAGE_VERSION + 1` and the other renegotiates. Generalised in the round to an **expiring-integer rule**: every version row states what the RFC *does not move*, never what a number currently reads. **Implementation is gated on D187** (`Chessboard.svelte` is destroyed and recreated on every node change), which is codex's, not this RFC's |
...
| 0.26 | `claim-backing.md` | **RE-CLAIMED 2026-08-16, reversing its own release.** Round 2's routing rule needs somewhere to put a principle id: `evidenceTypes` is a bare enum, so `$defs/feedbackClaim` gains `principles`. A ledger-side home was rejected on measurement — **18 of 81 `author_principle` uses are in packs with no ledger at all**. The lane also flips that `$def` to `additionalProperties: false`, **closing D112 at zero cost**: all **182 of 182** claims have the key set exactly `{id, text, evidenceTypes}`. The draft's *"no committed pack byte changes, no digest moves"* is **withdrawn**; D97's remedy still ships without a format version, and 0.26 buys the safeguard on the rung the routing demotes to |
