...
| `dead-vocabulary.md` | draft (awaiting cross-review) | — | **claims nothing versioned.** Cluster E's four defects are all owned by `format-surface`, so this RFC **cedes them and claims none**; it takes instead what that RFC declined — a **declaration census** (`DECLARATIONS=1`): producers / consumers / corpus-firings per declaration, report-only, zero never a verdict |
