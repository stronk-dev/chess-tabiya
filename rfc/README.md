...
| `runtime-opening-identity.md` | **accepted 2026-08-23 by claude as register owner, after an independent cross-review that failed 9 of 22 claims, two of them blocking. `vendor/` DOES NOT EXIST**, so §1.1's present-tense claim that a clean checkout rebuilds without a network request was false — the only reader fetches from `raw.githubusercontent.com` (`openings.ts:97`); it is now the first implementation obligation, with the five SHA-256 values pinned. Criterion 2's shared-parser rule was **unsatisfiable at HEAD** (`parseRows` is module-private while `normalizeOpeningPgn` is exported, so a compiler could satisfy it only by duplicating the parsing it forbids). §1.2 never said path keys **exclude the initial position** — the omission that decides whether 3,810/7,854 reproduce — so criterion 1 now also asserts the **2,023** maximum. Criterion 14's 2 ms p95 **could not fail** (a linear scan of all 7,854 keys fits inside it); now 50 µs p95 plus size-independence. Re-derived clean: `CHESS_OPENINGS_COMMIT` byte-exact, 3,810 / 7,854 / 2,023, and 401 of 6,991 recomputing to 5.7%. Law 8 clean — ECO/name always carry source id, commit, digest, file and row. Compiles the already-pinned CC0 Lichess catalogue into a deterministic local artifact and keeps exact current named endpoint, current path membership without a guessed descendant, and retrospective deepest reached separate. No sticky label, “out of book,” book policy, theory prose, LLM/FTS applicability or longitudinal claim | `planning/evidence-foundation-ux/` C3/F4/F7 handoff | **claims nothing versioned** — private server artifact; three opening projections plus the narrow recorded-position input |
...
| `exact-legal-mobility.md` | **accepted 2026-08-23 by claude as register owner, after an independent cross-review that failed 8 of 20 claims — the center one a FALSE STATEMENT ABOUT EXISTING BEHAVIOR.** §1.2's *“prove public behavior byte-identical”* is false: chessops emits `e1a1`/`e1h1` for castling and **never** `e1g1`/`e1c1`, so web input (which normalizes at `board-input.ts:205-207`) and server sourcing (`sourcing/legal-moves.ts:12-27`, which does not) **already disagree on every castling move**, and criterion 7 correctly **fails at HEAD**, turning green only on the real change. **The castling UCI normalization is accepted as a deliberate, content-visible behavior change, not a refactor** — taken because the two layers already disagree and one is wrong, with the blast radius **measured**: exactly one `uci` assertion argument in all committed sidecars (`philidor-third-rank-hold`, `h6h8`, not castling), so **zero committed bindings change validity**. §3's binding route ran through a list derived from the **pack schema's** `STRUCTURAL_FEATURE_KINDS` — closed to this projection, which also proves nothing auto-binds. Criterion 12 was **unsatisfiable by identity** for the color-flipped clone enumerators (the [[D984]] class), now split in two; census counted at **14** production `allDests()` sites, 11 needing classification. Splits exact actual-turn legality from the convention-grounded B/N/R/Q safe-mobility payload; retains all six roles, en-passant and four promotion identities; unifies evidence, board input and server successor walking on one runtime authority. No safety, quality, recommendation or preset is inferred | `breadth-collectors.md` §3.2; F1; accessible board input | **claims nothing versioned** — one additive `rules.mobility` projection and an exported runtime helper, no schema/migration lane; the closed route through the pack schema is **refused rather than claimed** |
...
| `theming.md` | **awaiting D3 — implementation complete 2026-08-23; all code, Settings, documentation, and criteria 1–13 landed.** Three independent axes ship as 3 app themes × 2 board skins × 2 piece sets, with a validated unversioned global preference, device-mode default, cross-tab live updates, None/Fast/Normal movement, reduced-motion enforcement, stable board identity, split skin/evidence paint, exhaustive artwork licensing, token/literal/contrast/separation sweeps, and real post-gesture browser interpolation. D4 and play-composition D3 are discharged; the active holds are the owner-tier Settings intent amendment (this RFC D3 / [[D987]]) and the owner-use felt-quality pass (D5 / [[D840]]), not implementation. | `planning/theming/` | **claims nothing versioned** — client-only CSS + one plain validated `tabiya.theme` localStorage key (global, deliberately NOT the per-context assistance/workflow grammars); no schema, no migration, no lane; verified against all six registers with the draft present (`register-check`: 0 errors) |
