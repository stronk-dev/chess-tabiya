...
| `evidence-contract-manifest.md` | **implementing — returned at the second bind stage 2026-08-21 on [[D669]]/[[D670]], after the amended sentence/derivation path proved buildable.** The 25-row census includes `guidance.packet`, which §10.2 explicitly says is not a consumer, and anchors `analysis.engine` to a request made before its evidence exists; criterion 7 cannot truthfully make either accept an admitted view. Separately, 18 leaf predicate ids cannot identify the recursive multi-family `StructuralExpression` payload the same criterion requires wrapped. The valid amendment work is retained: raw packet prose deleted; runtime-sealed admitted/rendered views; one provider/checker authority; compare/story record and derived identities; reasoning review outside `VoiceScope`; detector/authored phase split; derivation widening checks. See `planning/evidence-contract-manifest/second-author-return.md`. D667/D668 untouched | reconciles the evidence seams of six implemented RFCs | **claims nothing in the six shared-resource registers.** Declaration IDs are an extensible, independently versioned set; no pack/run/shape/principle schema or storage migration |
