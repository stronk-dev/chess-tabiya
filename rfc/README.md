...
| `live-marker-quality.md` | **implementing — owner amendment 2026-08-16**: the 2026-08-15 accepted cost narrows from *"the marker leaves participants and spectators entirely, on every run, **permanently**"* to **"for the duration of live play"**. The 2026-08-15 record is left intact and the amendment stated beside it. **Live play unchanged**; the third-value permission **stays refused**. **Criterion 6 changes in two clauses** (*"non-reviewing spectator"*), and its `liveMarkers` half is an `AssistanceContext` object-literal test with no session layer, so compatibility is held by fixture convention | — | **claims nothing versioned**. Narrows live `irreversibility` to `last_of_role`, makes pivotal rendering exhaustive, and applies the human-split permission to the client marker plus `/voice` and `/speech` delivery paths. Proposes **L1–L6**, the standing admission rule for the live surface |
...
| `opponent-contracts.md` | **CLEAR TO ARCHIVE 2026-08-16.** Independent review returned *archive-with-follow-ups* conditional on one body correction — **A10 fired and was scored a pass** (won-root enrichment 1.571× → 1.178×, from an interval excluding uniform at p = 0.0101 to one covering it at p = 0.414) — landed at `e0ae0b2`. Its blocking follow-up [[D452]] (the fullmove-counter determinism bug **the remedy itself introduced**) is **closed** at `d77a9f1`, together with D453/D454 and the remainder of D382. D455/D456/D458 closed at `2d564cb`; **D457 stays open** — the census keys on rounded `dtz` where the runtime uses `preciseDtz`, so the *delta* stands and the *tie counts* are unvalidated | `planning/opponent-contracts/` | Run schema **0.17**, migration **23**, measured Maia resistance at mode scope, total opponent-mode dispositions, and neutral perfect-tablebase residual ties. **No pack lane**; 0.28 remains free |
...
| `feedback-delivery.md` | **OWNER-GATE CLOSED 2026-08-16; ready for a final author pass, then acceptance.** The narrowed Open question 1 is ruled: **ship the surface, then run the binding wave before anyone plays** — neither arm, both in order. Both arms had been priced on losses one fact removes: **nobody has played a run since 2026-08-12**. Three obligations ride with it ([[D462]]): Job 1 is **not complete when it ships** and its acceptance record carries the delivered share at the **end** of the wave, not day zero's 98/196; **the wave is not only sourcing** — 63 of the 98 withheld claims trip `CLAIM_AUTHOR_LABEL_REQUIRED` and need a **pack edit**; and the no-learner-sees-it property **expires** the moment somebody plays before the wave completes. Cross-review verdict was accept-after-corrections and the corrections are in the file | — | **claims nothing versioned**, and takes **no migration position** — deliberately, so it does not join [[D423]]'s contest, now two-way after `opponent-contracts` landed. C7 turned out to have **already shipped** (`PackRecord.boundClaimIds` + `claimBackings`), so it adds no field to `PackRecord` at all |
