...
| `evidence-contract-manifest.md` | **amended and re-accepted 2026-08-21 by claude as register owner, on the buildability test, after the implementation return and a second adversarial cross-review.** The amendment closes [[D662]]'s two-authorities bypass by **deletion, not filtering** — the packet's raw `sentences` array is gone; deterministic text, provider input and `voiceCheck`'s allow-list derive from one brand-sealed view. **The review found the next bypass in the exact spot the last one lived**: rendered items were unbranded, so a call site could pair admitted evidence with arbitrary sentences, fully typechecked — sealed now at the sentence layer, with the brand a **runtime property** so `as unknown as` double-assertion fails too. `derived.story@1`'s declared inputs were **false at the symbol** — `story.rank` reached three surfaces undeclared; now `derived.story.rank@1` with truthful inputs, and the law-8 verdict is clean: a fixed product-convention ordering is the pivotal-marker class, not R3/R11's refused statistical ranking — **the refused shape is an UNDECLARED prominence ranking, and that is now fenced**. All four negative fixtures verified genuinely red at the checkpoint. Census **25**; reasoning review leaves `VoiceScope`; the payload-forge residual is deferred to F2 **by row** ([[D668]]), not by silence | reconciles the evidence seams of six implemented RFCs | **claims nothing in the six shared-resource registers.** Declaration IDs are an extensible, independently versioned set; the compiler rejects duplicate `(id, version)` pairs. No pack/run/shape/principle schema or storage migration |
