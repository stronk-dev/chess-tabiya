...
| `tactical-collectors.md` | **draft 2026-08-22 — the Phase-2/3 collector family, built strictly from the Phase-2 audit and the [[D718]] rulings.** Twenty-nine projection ids in a closed Appendix-A table that criterion A1 counts; six conventions pinned by exact values (`see-convention@1` 1/3/3/5/9; `space@1` per the ruling, cited `chess_tradition`; `threat@1` null-move+SEE+mate-in-1). Build order SEE-first. Per-collector lift acceptance **with sign**: hanging is predeclared negative-primary (0.26×/15.7% — the ruled avoidance form as primary reading), islands and rook-7th positive, rays and space land as state/level with deltas demoted, capture carries deliberately **no** lift claim, and A9 states **both fork branches** per the pre-authorized fallback. Five fixtures start red at HEAD, and the one guard that cannot fail at landing is **labelled as such so it is never scored as evidence**. All families land **research/inspector-only** — production eligibility is Phase 3's, which keeps *registered ≠ usable* structural. Surfaced honestly: the audit reserved no row for fork (assigned D745 at landing) | `planning/evidence-foundation-ux/` | **claims nothing versioned — verified, not assumed**: the draft deliberately does NOT extend `STRUCTURAL_FEATURE_KINDS` (each member is a pack-schema arm behind live lanes 0.28/0.29); all families are catalog-local `@1` identities on F1's extensible set, the F2 precedent, with the authorable-vocabulary admission deferred to a named follow-up. `register-check` green with the draft present |
