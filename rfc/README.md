...
| `runtime-opening-identity.md` | **draft — returned 2026-08-23 for a factual source-pin amendment.** First implementation fetch reproduced four hashes and proved the accepted `b.tsv` SHA was mistranscribed; D1052 records the independently verified upstream/source-register value. Projection semantics and scope are unchanged; independent refresh is required before implementation resumes | `planning/evidence-foundation-ux/` C3/F4/F7 handoff | **claims nothing versioned** — private server artifact; three opening projections plus the narrow recorded-position input |
...
| `exact-legal-mobility.md` | **draft — amendment 2026-08-23 returned for cross-review before implementation closeout.** The earlier accepted review correctly found web/server castling disagreement but owner ruling [[D1029]] reverses its remedy: move identity stays Chess960-safe king-to-rook; semantic destination is the king's c/g landing square; display stays SAN. A direct consumer audit found the old RFC also missed last-move highlighting, comparison piece routes and semantic-event canonicalization, each of which conflates raw UCI target with piece destination. The amendment makes all four able to fail and adds a nonstandard Chess960 fixture. Non-castling scope remains unchanged: exact actual-turn all-role legality, en-passant/four-promotion identity, strict adapter, inspector-only projection and a 14-site enumerator census. No safety, quality, recommendation or preset is inferred | [[D1029]], `planning/exact-legal-mobility/d1029-consumer-audit.md`; `breadth-collectors.md` §3.2; F1; accessible board input | **claims nothing versioned** — one additive `rules.mobility` projection and exported runtime helper; no schema/migration lane |
