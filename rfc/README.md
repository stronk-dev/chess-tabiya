...
| `pack-capability-contract.md` | **draft — returned to author 2026-08-23; cross-review found six return-class blockers** (~65 claims checked, 17 failed). **The mechanism holds and the [[D566]] test is real, not decorative**: criterion 13's three predicate-bearing documents are exactly `content/shapes/{knight-vs-bishop,maroczy-bind,open-centre}.json`, all inside §7's included roots, and `digest.ts:69` digests every byte with no field filter — so nothing shipped would have caught `pawn_safe_square`'s drift and a source-region digest does. Lane 0.30 is genuinely next-free, the claims block joins character-for-character, and every §7 population count reproduces. **What returns it:** (1) **criterion 4 pins a census that is wrong at source** — `BREADTH_CONVENTION_TEXT` has **8** entries at HEAD (`evidence-catalog.ts:183-190`), so conventions are **13** not 12 and `89+60+13+12+17=191` is satisfiable **only by an implementation that drops a convention** ([[D984]]'s class); the wrong figure is in the Summary, §2.4 **and this row**, so the repair is one commit or it creates a body/register split. (2) **No closure argument, and the enumeration is provably open** — `SimpleTrigger` (6 arms, `packages/schema/src/drill-pack/types.ts:82-89`) has **no census row** *while §2.5 names it as one of two twice-interpreted vocabularies whose two sites criterion 6 asserts*, `TransitionExpression` (5) likewise, and `rules_fact`/`fenPredicate` are **double-counted** as standalone vocabularies though they are fields inside other arms; the fix is a **closure procedure over the schema's 52 `$defs`**, not a hand-count. (3) **Four of criterion 4's five terms are arithmetically wrong**: §3a sums to **90** not 89, §3a-ter to **62** not 60, §3f has **16** rows not 17. (4) **Criteria 8 and 16 are mutually unsatisfiable** — C8 demands `packCapabilities` set-equal the registry, C16 + [[D509]] demands an unsupported capability be **absent** from it, and every deployment carries one (`plan_defense`/`human_external` ship `refused`; `ENGINE_MODE=mock` drops providers). (5) **§4.4 does not satisfy `claim-semantic-anchors` §7 and its "byte-for-byte" table is false** — §7 dispatches on a `contract` key per *binding object*, §4.4 on a `requires` array at the *sidecar root*: different granularity, different refusal code, and the per-document form breaks §7's Stage-A/Stage-B split. (6) **`claim.binding` is required by §4.4 and absent from the enumeration** — register it and 191 breaks, don't and every sidecar declaring it is refused. **Gate F clauses 5 and 6 are NOT tickable on this draft** (clause 5 rests on criterion 16). Corrected in place: criterion 15's sidecar population **32 → 68** (the glob dropped all 36 candidate sidecars — §7's own defect class inside a criterion) and five citation errors, including *"refuse-to-serve, not degrade"* attributed to `docs/drill-client.md:16`, **which contains no such string** | F3 in `rfc-graph.md:70`; F1's manifest; `graduation-clearance` §6.5's planner precedent | claims **pack-schema lane 0.30** — `requires` (new, required) + `$defs/capabilityRequirement`; the stamp is inside `digestDrillPack`, so **all 92 pack digests churn** and every ledger re-stamps (Discharge D3, held behind [[D949]]'s graduation arm). **No migration position** — packs are files, not rows |
...
| `runtime-opening-identity.md` | **accepted 2026-08-23 by claude as register owner, after an independent cross-review that re-derived 22 claims and failed 2 — and the corrected `b.tsv` pin is confirmed WITHOUT trusting the fetch that produced it.** `git hash-object` reproduces the cited GitHub blob `c4c7f890…` locally from the vendored bytes, and the superseded value **shares its first 15 hex characters** with the true one — 60 shared bits, which no genuine SHA-256 of different bytes produces — so it was provably a corrupted copy, not a hash of anything. **Both failures were about the provenance of the correction, not the correction**: the return note mis-cited the accepted status (which said the hashes were *pinned*, never *re-derived* — the real defect is an **unverified authority made load-bearing**), and the single-source check itself, since **three in-repo witnesses carried the correct value all along**, including the production sourcing path's own committed `sources.json` manifests. **No production or content byte was ever wrong** — the defect was confined to a disposable harness README that criterion 1 reads; §1.1 and criterion 1 now require cross-witness agreement plus byte counts. All numbers recomputed from the vendored bytes: **3,810 / 7,854 / 2,023**, and 401-of-6,991 = 5.7% three independent ways. Criterion 6's max-descendant key is now literal (after `1.e4`); criterion 7 names its corpus file | supplies the runtime adapter split out of `tactical-collectors.md` §3.15 and discharges `semantic-collectors.md` D3 (amends neither); depends on implemented F1, the shipped `transposeKey` and the pinned `sourcing/openings.ts`; planning home `planning/evidence-foundation-ux/` C3/F4/F7 handoff | **claims nothing versioned** — private server artifact; three opening projections plus the narrow recorded-position input |
| `review-evidence-compiler.md` | **draft 2026-08-23 — executes Semantic Collectors D2 from the completed D916–D928/C4 evidence.** Replaces Story's mate→±1000-cp corruption with typed cp/mate points and transitions, normalizes Stockfish WDL perspective at the source boundary, and compiles partial per-family Review packets with explicit absence. It deliberately leaves final Review Map quotas/priorities on D928 because C4 measured overlap, not learner usefulness | `planning/evidence-foundation-ux/` C4/F6 handoff | **claims nothing versioned** — four manifest projections, additive event provenance, no schema/migration lane |
| `exact-legal-mobility.md` | **accepted 2026-08-23 by claude as register owner, after an independent cross-review of the [[D1029]] amendment that re-derived 22 claims and failed 7 — center catch: `isLegal` CANNOT POLICE THE DIALECT.** `isLegal(e1g1)` and `isLegal(e1h1)` are **both** true and play to identical successor FENs (`chess.js:333` falls back to `normalizeMove`), so §2's *"replays legally from the FEN"* rule admitted a wrong-dialect payload; conformance is now `makeUci(normalizeMove(pos, parseUci(uci))) === uci`. **[[D1027]]'s ingest half was deferred as "a future UCI join" and is false at HEAD** — engine `bestmove` is already in the other dialect (`UCI_Chess960` refused at `capabilities.ts:133`) and **21 castling `moveUci` values are committed across 13 packs**: two of three inbound populations, not zero (criterion 15). **Criterion 7 kept its teeth but lost the red-at-HEAD evidence proving it** — restored arm-by-arm, the *web* arm now the red one. Also: the census split broke to 7/7 against its own 8/6 enumeration; **[[D1028]] was flipped at closeout and fixed nowhere in the body** (negative control now stubs **both** filters, since a control downstream of them measures nothing); the blast-radius sweep priced the *reversed* direction; and the mandatory 960 fixture omitted its own degenerate case — `g1h1` is O-O with the king's semantic destination **equal to its origin**. 960-superset premise confirmed by running chessops (`normalizeMove(e1g1) → e1h1`; castling rights are a `SquareSet` of **rook squares**); 14-site census exact. **Named residue:** [[D1029]] names three layers and the RFC names two — `MOVE_DESTINATION_CONVENTION` exists but the field is bare `to` with no display constant; criterion 15 keeps the naming failable | follow-up to implemented `breadth-collectors.md` §3.2 (adds an exact projection, redefines neither `piece_destinations@1`); depends on implemented F1 and `accessible-board-input`; carries [[D1027]]/[[D1028]]/[[D1029]] and `planning/exact-legal-mobility/d1029-consumer-audit.md` | **claims nothing versioned** — one additive `rules.mobility` projection and an exported runtime helper; no schema, migration or lane |
