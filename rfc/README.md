...
| `breadth-collectors.md` | **draft — returned 2026-08-22 by Codex author-side buildability review; Phase-2b research remains complete; author amendment then independent Claude review are required before acceptance.** Eighteen ids remain the right bounded wave, but nine predicate/input seams are not executable from one meaning: legal-controller occupancy, exact defender-edge authority, pre-move contact execution, mixed sequence horizons, pressure-line identity/color, pawn relation boundaries, en-passant captured square and half-open-file color. Measured low/mixed atoms remain inspector/on-demand and the silent landing stands. Depends on tactical collectors for `legal-exchange@1` and `reply_breadth@1` | D802/D851–D860; `planning/evidence-foundation-ux/breadth-collectors-review.md`; completed Phase 2b dossier set | **claims nothing versioned**; no authorable enum, schema, migration, content or grounding change |
