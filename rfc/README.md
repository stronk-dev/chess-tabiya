...
| `teacher-surface.md` | **draft — OWNER BLOCK DISCHARGED 2026-08-16; the new §5.2 awaits cross-review before acceptance.** The block was Open question 1 — *does the review surface work without Maia and the corpus?* **The owner refused the fork and ruled that teacher mode ships complete**: *"why do you not give option: add them and ship no deferral"*. So the question is **dissolved rather than answered** — §5.2 now specifies the review rail (~230 lines, six criteria) and there is no longer a version of this RFC that ships without it. **One rule:** on a terminal, disclosed run with no live session open, a submission-granted teacher gets **the run host's own table**, never a reviewer tier — `reviewing` sits in the *role* disjunct, never beside `deliveryOpen`, so a reviewer can reach the run's disclosure line and never pass it (`design/05` §3a-i: *"the run — not the viewer — carries the barrier"*). Delivers **all seven** surfaces §2.3.4 had refused, the seventh arriving with **no code change** via `live-marker-quality`'s own L4(b). **One thing is refused as impossible rather than expensive:** a teacher watching a student play **live** still gets no Maia or corpus, because opening rung 3 to a live spectator would **reverse an owner ruling of 2026-08-15** on a surface another RFC is mid-implementation on — and an author does not reverse an owner ruling by specification. **§5.2 is 230 lines of new specification that the 2026-08-16 cross-review never saw**, which is why this is not yet `accepted` | — | **claims one migration position** (`STORAGE_VERSION + 1`; head is **23** at `6ba0736`) — `ALTER TABLE run_grants ADD COLUMN granted_via TEXT`, nullable, no backfill, no CHECK, chosen over a join on `assignment_submissions` because §4.1a already ruled the grant must outlive the record of the act. Four tables, `run_grants.expires_at`, `live_sessions.classroom_id`; **no run- or pack-schema change**, no new token scope, no fourth `RunRole`, no new session kind. The proposed swap with `board-annotation` is **withdrawn as moot** — that RFC is implemented and archived at migration 22. Now also claims **D92 and D93**, which both named this RFC as their owner and were never claimed |
| `opponent-contracts.md` | **implementing 2026-08-16 — pending independent review** | `planning/opponent-contracts/` | Run schema **0.17**, migration **23**, measured Maia resistance at mode scope, total opponent-mode dispositions, and neutral perfect-tablebase residual ties. **No pack lane**; 0.28 remains free |
| `graduation-clearance.md` | **accepted 2026-08-16 — ready for implementation.** Cross-reviewed and then author-rounded the same day; **all four author-call open questions closed in the body** (1, 2, 3, 7), question 5 deliberately deferred to the first content wave. **No owner ruling owed.** The acceptance gate — open question 4's residue — closed by **publishing the classifier ruleset** as a literal seven-rule keyword list (§5.1a) and enumerating what it actually produces: **17 residue entries, not 12**, and the four the cross-review cited as evidence were **matched by rule 1 and never residue** ([[D434]] — an unpublished ruleset manufactures its own residue). Six of eight cells moved; `clearable` **179 / 41**, and the pack split **inverts to 27 instrument-bound / 23 authoring-bound**. The three tightenings proposed against the acceptance back door **do not close it and one is unpayable** ([[D435]]); the closing check is a **line-level `git blame`**, with the residual floor stated — without a second party nothing closes further. The two absent lint codes are **handed back rather than minted**, because their conditions are unrepresentable under the closed schema | — | claims **pack schema 0.28** — *earned, not discretionary*: `$defs/provenance` **and** `$defs/graduationEntry` are both `additionalProperties: false` at 0.27, so there is no unversioned hiding place. **No migration position** — so [[D423]]'s three-way contest does not reach it — **no run schema, no shape-entry.** Completes `pack-graduation` by specifying the clearance grammar it deliberately left open: **`resolved` is a standing predicate a checker re-runs; `accepted` is a citation to a ruling recorded outside the pack; neither is ever an author's sentence about their own pack** |
| `measurement-records.md` | **returned to author 2026-08-16** (core sound; 3 open questions + D391 block acceptance) | — | claims **shape-entry schema 0.4** only — one optional `measurements` property + three `$defs`. **No pack lane (0.28 stays free), no run schema, no migration.** Splits the record into a normative half the gate may fail on (`textSha256` + span agreement) and a diagnostic half it may never fail on. **Cross-reviewed 2026-08-16: sound in its core, RETURNED TO AUTHOR** — three open questions must be ruled before `accepted`, and sub-expression readings (D391) were promoted to a blocker. The 4-vs-8 refutation **reproduces independently**; qualified in review to *4 errors + 8 one-flag warnings vs 12 indistinguishable failures* |
...
| `dead-vocabulary.md` | **accepted 2026-08-16 — ready for implementation.** Cross-reviewed the same day; all three Open questions are non-blocking by their own text. The cession's factual basis had **inverted under it**: §1 argued `format-surface`'s dead-vocabulary half was *"in flight, not landed"*, and all four dispositions had since shipped in committed `532c7e2` — replaced with a per-row **tree** verification table, since a specified-then-dropped disposition is the [[D400]] mode. Four **specification** defects in the census design fixed (schema rows are *bare* pointers, not `schema:`-prefixed, so a uniform namespace join matched nothing; the producer definition missed two real syntactic forms, giving **3 false zero-producer codes** in a deliberately naive run; a refusal emitter counted as a consumer — [[D429]]; `tools/` was never excluded, and `vacationReading` is the case that proves it must not be). Two criteria were **unsatisfiable as drafted** — both told the implementer to flip a D360 row already flipped. **D360's refutation re-derived independently: 265/771, identical** | — | **claims nothing versioned.** Cluster E's four defects are all owned by `format-surface`, so this RFC **cedes them and claims none**; it takes instead what that RFC declined — a **declaration census** (`DECLARATIONS=1`): producers / consumers / corpus-firings per declaration, report-only, zero never a verdict |
...
| `feedback-delivery.md` | **cross-reviewed 2026-08-16 — accept after corrections, and OWNER-GATED again on one narrow question.** The author round's closure of Open question 1 was **wrong on its third ground and is now narrowed rather than closed.** The 2026-08-15 ruling says in its own words *"the owner refused **all three**"* — **C6 included**, at a measured 49.0% delivered share — so *"the owner already ruled"* and *"the owner ruled for C6"* are different sentences and only the first is true. What the ruling **does** settle: the three-way fork is dead and must not be re-asked. What it does **not** authorise: shipping C6 at 50.0% with the binding wave unrun. **The question that survives is a SEQUENCING one** — does Job 1 land ahead of the binding wave or behind it — with both sides priced in §3.2 and the author's position stated as a position. **The residue is also larger than the author round found** (§3.2a, new): `validateClaimBindings` admits only under `if (issues.length === before)`, so severity is irrelevant, and **83 of the 98 withheld claims lack `author_principle`, 63 provably tripping `CLAIM_AUTHOR_LABEL_REQUIRED`** — their remedy is a **pack edit, not a ledger entry**. *"No class is structurally unbindable"* is true and insufficient: **the checker refuses by sentence shape, inside every class.** Three further blockers fixed in place — C8 rendered a **false provenance line** on the only claim that can fire it today (19 of 99 machine-labelled claims carry a non-machine label); criterion 2a was **unsatisfiable** ([[D444]]); and §2.2c's `reachableAuthoredSpineIds` explanation was **factually false** (5 of 50, not all 50 — and correcting it *strengthens* the case). **Every one of ~40 measured figures reproduced independently, twice** | — | **claims nothing versioned**, and takes **no migration position** — deliberately, so it does not join [[D423]]'s contest, now two-way after `opponent-contracts` landed. C7 turned out to have **already shipped** (`PackRecord.boundClaimIds` + `claimBackings`), so it adds no field to `PackRecord` at all |
...
| 0.23 | `engine-leverage.md` | implementing 2026-08-16 — `guard.conditions[]`, `$defs/engineCondition`, a fourth `deviationCost` arm |
| 0.24 | `vocabulary-wiring.md` | implementing 2026-08-16 — `plan_signature` leaf on `$defs/structuralExpression`; deprecates `plan_consequence` |
...
| 0.28 | `graduation-clearance.md` | **claimed 2026-08-16 and kept** — *earned, not discretionary*: `$defs/provenance` **and** `$defs/graduationEntry` are both `additionalProperties: false` at 0.27, so five `$defs`-level changes have no unversioned hiding place. Adds the clearance grammar — `subject` as a single JSON pointer, the seven-kind taxonomy with `unbuilt` separated from `unreachable`, and `clearedBy` given a checkable shape. **No migration, no run schema, no shape-entry** |
| — | **0.28 is the next free pack lane** | recorded 2026-08-15 after a cross-review found three drafts holding lanes with no register rows — the exact collision class this register exists to prevent |
...
| 23 | 22→23 | `opponent-contracts.md` | **implemented 2026-08-16** at `6ba0736`; `STORAGE_VERSION` is **23** at HEAD, so [[D423]]'s contest on `STORAGE_VERSION + 1` is now **two-way, not three** ([[D447]]). RFC stays active pending independent review — stamp-only run schema 0.16→**0.17**; optional `OpponentSelection.orderingBasis`, historical selections remain absent and are never inferred. Body uses frozen literals |
