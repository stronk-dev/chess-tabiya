...
| `semantic-evidence-selection.md` | **accepted 2026-08-21 on the owner's continue direction after two buildability reviews and D684's retained external fixture repair.** Adds 22 exhaustive identity-preserving direct event projections, eleven registered denominator-bearing derived `avoided` projections, exact consumer eligibility, a parameterised local legal-alternative selector and the D668 payload seal. It leaves tactic/plan labels, production module defaults, learner-visible routing, persistence and all content migration out. D660 is corrected: the older 294× global-lift result remains diagnostic and is prohibited from product selection; the later R2 local selector is the transferable mechanism | `archive/evidence-contract-manifest.md`; structural/transition semantic residue | **claims no shared resource, schema lane or migration.** Extends the compiled manifest with independently versioned declarations; no production policy is selected until F5 |
