...
| `teacher-surface.md` | **cross-reviewed 2026-08-16 — accept after corrections (made in the file), plus ONE small owner confirmation.** The owner block is discharged: the ruling *"add them and ship no deferral"* dissolved Open question 1, and §5.2 specifies the review rail — a submission-granted teacher gets **the run host's own table**, never a reviewer tier. **The design survived; the author round's three supporting arguments did not**, each failing in a way the standing-hazard list names exactly. **(1) A reviewer could see strictly MORE than the run's own host** — `seatedInContest` was defined with no time bound, and `match_states` rows are `ON DELETE CASCADE` from `live_sessions`, which are **closed and never deleted**, so the seat was permanent: on a finished native match the host read `false` on their own game forever while a teacher read `true`. The deep-equality criterion passed because its fixture was a solo pack, where every candidate implementation agrees — the [[D444]] shape. Bounded by `closed_at IS NULL`, which §5.1's own prose already said. **(2) *"Compatible by construction, not by fixture edit"* was false** — `live-marker-quality`'s criterion 6 is an object-literal unit test with no session layer at all; the fixtures stay green by **fixture convention**. **(3) §5.3 cited a ruling that says the opposite** — the same failure as `feedback-delivery` today: §5.2 was already placing the live marker for a reviewing spectator while §5.3 argued the RFC may not cross that ruling. The review boundary moves on the **2026-08-16** ruling's authority; the live boundary has none behind it. **The edits owed to `live-marker-quality` go from one word to three sentences, one of which is the owner's recorded ruling terms — that is the confirmation owed before this returns to `accepted`** | — | **claims one migration position** (`STORAGE_VERSION + 1`; head is **23**) — `ALTER TABLE run_grants ADD COLUMN granted_via TEXT`, nullable, no backfill, no CHECK. §4.3's write table is now **seven rows / twelve statements**, counting *statements* rather than sites, because every miscount across three revisions had one shape: a function listed while one of its two statements was not ([[D460]]). Four tables, `run_grants.expires_at`, `live_sessions.classroom_id`; no run- or pack-schema change, no new token scope, no fourth `RunRole`. Also claims **D92** and **D93** |
