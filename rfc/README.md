...
| `tactical-collectors.md` | **draft amended 2026-08-22 — prerequisites measured; refreshed independent buildability review pending.** Thirty scoped projection ids: six conventions plus exact `reply_breadth@1` and check. D730 measures en-prise 0.36×/0.57× and meaningful double attack 1.72×/1.96×. D794 finds threat presence background (0.91×/1.04×), the same positive capture survives every reply only 1/0 times, and `fork_survives_reply@1` holds on 0/10 authored and 2/29 imported played fork events. Exact replies/refutations remain; forcing/winning language does not. All families land research/inspector-only | `planning/evidence-foundation-ux/tactical-collectors-review.md`, `design/research/legal-exchange-prerequisite.md`, `design/research/bounded-reply-semantics.md` | **claims nothing versioned**; authorable vocabulary remains separate |
| `breadth-collectors.md` | **draft 2026-08-22 — Phase-2b research complete; independent buildability review required.** Eighteen closed projection ids cover exact square controllers, per-piece legal/local-safe sets, pawn contacts/candidate/passage, retained pawn/defender sequences, material-role state, king zone/shelter/escapes and open-file occupancy. Existing pawn-contact, passed-pawn, piece-count and open-file authorities are consumed rather than duplicated. Measured low/mixed atoms remain inspector/on-demand; sequence order never becomes force or causality. Depends on tactical collectors for `legal-exchange@1` and `reply_breadth@1` | D802; completed Phase 2b dossier set | **claims nothing versioned**; no authorable enum, schema, migration, content or grounding change |
