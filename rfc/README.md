...
| `graduation-clearance.md` | **cross-reviewed 2026-08-16 — returned to author, then accept.** Four blocking corrections, all specification gaps rather than reasoning errors, fixed in the body. **Headline refuted 3 → 0 of 50** (23 instrument / 27 authoring) — and losing it *strengthens* the RFC. **0.28 KEPT** and verified: `$defs/provenance` and `$defs/graduationEntry` are both `additionalProperties: false` at 0.27, and this claim carries five `$defs`-level changes — **the opposite shape to the register-only claim released from `opponent-contracts`** | — | claims **pack schema 0.28** — *earned, not discretionary*: `$defs/provenance` **and** `$defs/graduationEntry` are both `additionalProperties: false` at 0.27, so there is no unversioned hiding place. **No migration, no run schema, no shape-entry.** Completes `pack-graduation` by specifying the clearance grammar it deliberately left open: **`resolved` is a standing predicate a checker re-runs; `accepted` is a citation to a ruling recorded outside the pack; neither is ever an author's sentence about their own pack** |
