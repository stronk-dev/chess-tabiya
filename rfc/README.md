...
| `breadth-collectors.md` | **draft amended 2026-08-22 — Codex repaired all nine author-side returns; independent Claude review remains before acceptance.** Eighteen ids remain the bounded wave. Actual legal destinations now define legal control; exact controller/contact authorities and two-/three-edge horizons are pinned; pressure/pawn boundaries copy the harness; en-passant square and mover-relative half-open joins are explicit. D851–D859 remain open until independently reproduced. Measured low/mixed atoms stay inspector/on-demand and the silent landing stands. Depends on tactical collectors for `legal-exchange@1` and `reply_breadth@1` | D802/D851–D860; `planning/evidence-foundation-ux/breadth-collectors-review.md`; completed Phase 2b dossier set | **claims nothing versioned**; no authorable enum, schema, migration, content or grounding change |
