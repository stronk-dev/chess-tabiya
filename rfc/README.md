...
| `theming.md` | **accepted 2026-08-22 by claude as register owner, after three passes: cross-review, a RESTRUCTURE on owner ruling [[D982]] (*"have app theme, board theme, pieces theme. done."*), and an independent verification (138 claims, 12 failed, all corrected).** The first cross-review found **both color criteria unsatisfiable as drafted** — one demanded a color distance between the four brushes and a "lighting overlay" that IS the blue brush (`DrillScreen.svelte:374`, distance 0.0 by identity), the other imposed WCAG floors **nobody had ever computed** (two of four shipped palettes failed). The verification pass then caught a criterion that would have been **red forever** (`--display-font` evicted from the palette with 23 uses admitted by nothing) and an **invalid-CSS mapping** (`--shadow`'s seven call sites are `box-shadow:` shorthand consumers, so a 1:1 map to the registry's bare `shadowColor` would emit `box-shadow: rgba(...)`; split into `--shadow-color` + derived geometry). **The ruling's own vindication is [[D988]]**: the composed contract let the first reviewer edit `accentTextColor` believing it was ours — violating the verbatim rule in the sentence that cited it. Verified at source: all 12 contract keys present in the owner's registry (all 10 schemes carry all 12 — the contract really is inherited vocabulary), all 24 `tokyo-night` values byte-exact upstream with zero drift, every contrast and ΔE figure reproduced exactly. **The first draft was RETURNED because it composed:** it defined a 17-token contract of Tabiya's The first draft was RETURNED because it **composed**: it defined a 17-token contract of Tabiya's own and filled it by pairing inherited *foregrounds* with Tabiya *surfaces* — two color schemes in one screen — whose visible symptom was a WCAG question escalated to the owner (*which of our surfaces should Tokyo Night's red sit on?*). The owner's answer was that the question should not exist. Now: **three orthogonal axes** (`appTheme` / `boardTheme` / `pieceSet`, no cross-axis composition rules, enforced by new criterion 13 — switching one axis leaves the other two byte-identical), and the token contract is **the scheme's own vocabulary** read from the owner's registry: **12 tokens, every one a key real schemes already ship**, down from 17 authored — retiring `--paper-soft`/`--panel-soft` (8 call sites migrate), moving `--board-light`/`--board-dark` to the board axis, deriving `--scrim` once instead of authoring it per theme. Palettes are **transcribed whole, backgrounds included**, so `origin: "tabiya" | "inherited"` is a required field and **§3.3 gates our own palettes while measuring-and-publishing inherited ones** — a criterion satisfiable only by editing an inherited scheme would contradict [[D976]] and is the [[D984]] class. **The escalated open question is therefore DISSOLVED, not answered.** The restructure also caught its own predecessor: the previous cross-review edited `tokyo-night` light's `on-accent` `#ffffff` → `#0d0e14` reasoning it was *"Tabiya's own token"*, but the scheme carries `accentTextColor: '#ffffff'` — it **violated the verbatim ruling in the act of citing it**; restored, and criterion 5b's committed measurements table now fails on any silent edit to an inherited palette (row C). Load-bearing engineering unchanged: `chessground.brown.css` fuses square color with all CSS-painted evidence paint, so board themes become skin-only over one shared token-driven `interaction-paint.css` — the split is what makes the board a real axis rather than a swap that silently repaints evidence (brush colors are a JS `config()` seam, not in that file). [[D977]]'s two strips hold (no version machinery on `tabiya.theme` — validation IS the migration strategy; no license criterion on a palette — criterion 11 is artwork-only). **[[D983]]'s live defect is fixed here, not deferred**: the shipped light theme's `--warning` is **1.94:1** on `--paper` while rendered as small text in eight places, and `paper` is `origin: "tabiya"`, so §3.3 gates it | `planning/theming/` | **claims nothing versioned** — client-only CSS + one plain validated `tabiya.theme` localStorage key (global, deliberately NOT the per-context assistance/workflow grammars); no schema, no migration, no lane; verified against all six registers with the draft present (`register-check`: 0 errors) |
