...
| `accessible-board-input.md` | **draft 2026-08-21 — F12-F, the independent accessible-input child of the Choice-C appliance.** One board-input controller projects click/drag/touch, keyboard square navigation, an assistive semantic grid and typed SAN/UCI entry to the existing validated UCI submission. It preserves the current 90/90 pointer/touch floor, expands it to a 150-cell five-input matrix, restores normal Tab/Shift+Tab traversal and requires owner validation on available devices before 1.0. It changes no evidence, assistance, schema, storage or content contract | `archive/client-surface-floor.md`; F12 work order | claims nothing versioned; web input/accessibility code, tests and docs only |
| `evidence-contract-manifest.md` | **draft 2026-08-21 — F1, opened by O1 after A0-A4/R1-R2 and the protected intent/process predecessors landed.** Compiles versioned producer → projection → consumer declarations for all fourteen audited producer paths; separates predicate, reading and event identities; requires exact consumer/timing/role/session/form/answer-distance bindings or an explicit inspector/author/operator/experimental/retired disposition. The author buildability pass fixes the shared-runtime catalogue/server-availability split and publishes an eighteen-operation current-consumer census; cross-review must re-derive both. It deliberately does not choose semantic learner events, selection, modules, presets or defaults | reconciles the evidence seams of six implemented RFCs | **claims nothing in the six shared-resource registers.** Declaration IDs are an extensible, independently versioned set; the compiler rejects duplicate `(id, version)` pairs. No pack/run/shape/principle schema or storage migration |
