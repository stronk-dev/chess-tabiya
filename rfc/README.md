...
| `evidence-contract-manifest.md` | **implementing — second author-return amendment accepted on buildability 2026-08-21; bind stage resumed.** [[D669]] now distinguishes producer/computation, provider acquisition and consumer/delivery: the packet constructor and POST `/analysis` are separately anchored but absent from the 23-operation consumer census; completed analysis is delivered through `runtime.evidence_ref`. [[D670]] now gives the complete authored structural AST, direct feature-leaf results and total traced predicate result separate identities. Runtime typecheck, 8 focused tests and `make evidence-manifest-check` are green (17 producers / 80 projections / 23 operations / 197 bindings). Remaining work is real consumer-view wiring/type fixtures, not another owner ruling. D667/D668 untouched | reconciles the evidence seams of six implemented RFCs | **claims nothing in the six shared-resource registers.** Declaration IDs are an extensible, independently versioned set; no pack/run/shape/principle schema or storage migration |
