...
| `opponent-contracts.md` | **accepted 2026-08-16 — owner ruled OQ2 in the body; READY** | — | **cross-reviewed 2026-08-16: accept.** Claims **run schema 0.17** (`OpponentSelection.orderingBasis` — earned, because `$defs/opponentSelection` is `additionalProperties: false`), a **stamp-only migration at position `STORAGE_VERSION + 1`** (contested with `teacher-surface`; this RFC yields), and an unversioned `/capabilities` change. **NO pack lane — 0.28 released and remains free**: the cited `claim-backing` precedent does not exist (that commit never touched `dispositions.ts`) and every pack lane 0.3→0.27 carries a `$defs`/enum/property change. Names the **third direction** of the declared-vs-executable law: executed, load-bearing, never stated |
