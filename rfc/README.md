...
| `tactical-collectors.md` | **draft amended 2026-08-22 — prerequisite measured; independent buildability review pending.** Twenty-eight scoped projection ids in a closed Appendix-A table; six conventions pinned, led by `legal-exchange@1` (legal recapture-only minimax, P1/N3/B3/R5/Q9). D730 measures en-prise as robustly negative-primary (0.36×/0.57×, both intervals below 1) and exchange-filtered fork at 1.72×/1.96×; the authored fork interval crosses 1, so the exact event has no universal positive-primary disposition. Threat now abstains while in check, trapped destinations are legal/exchange-tested, and local fork consequence no longer claims “wins material.” Runtime opening identity was split back to gated D743/R8/F7. Ten normative production sites are closed in §1.1. All families land research/inspector-only; production modules remain Phase 3 | `planning/evidence-foundation-ux/tactical-collectors-review.md`, `design/research/legal-exchange-prerequisite.md` | **claims nothing versioned**: no `STRUCTURAL_FEATURE_KINDS`, schema, content or shared-grounding change; authorable-vocabulary admission remains a named follow-up |
