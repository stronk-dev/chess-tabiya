...
| `tactical-collectors.md` | **implementing 2026-08-22 — owner accepted the repaired 30-projection foundation wave after D730/D794 and independent Codex buildability review.** Six conventions plus exact `reply_breadth@1` and check; all families land research/inspector-only. D824/D825 bind every id to a literal manifest role and keep recorded salience out of the local rules payload. Module promotion, presets, authored vocabulary and opening identity remain explicitly outside this landing | `planning/tactical-collectors/plan.md`, `planning/evidence-foundation-ux/tactical-collectors-review.md`, `planning/evidence-foundation-ux/collector-rfc-joint-review.md` | **claims nothing versioned**; authorable vocabulary remains separate |
...
| `longitudinal-store.md` | **draft 2026-08-22 — the thrice-named blocker gets its document: the store is a PROJECTION of the event log, never a second source of truth.** Two strict double-cascading tables; the grain makes a denominator-free row **unrepresentable** (`CHECK (opportunities > 0)`); **no cross-game total is ever stored** — windows compute at read, because a fact you never write cannot go stale; `make longitudinal-rebuild` asserts byte-equality between incremental and replayed state with a mandatory tamper fixture; **no backfill by argument** (backfill IS rebuild at current code; freezing a semantic derivation into a migration body is the frozen-literal hazard). Privacy pinned at the schema: no vector column exists, both read paths learner-scoped, export owner-only, deletion hard-cascades with `__legacy` retention **refused**. `session_kind` includes `'imported'` — the deliberate difference from `attempts`, verified at the symbol. Three open questions declared for the accept gate; the two reviewer-pressure points named in advance (the `decisions` denormalization; whole-run-replace cost on large imports) | `planning/evidence-foundation-ux/` | claims **one migration position, `behind learner-rating`** (the position grammar avoiding a C3 collision at HEAD `STORAGE_VERSION` 24); nothing on the other five registers; ingest set referenced **by symbol**, not copied |
