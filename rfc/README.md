...
| `dead-vocabulary.md` | **implementing 2026-08-16 — pending independent review.** The opt-in declaration census is implemented with live-source mutation controls over all four namespaces, refusal sites separated from consumers, and the D360 correction reproduced at **265/771**. Full gates are the landing checkpoint; archival waits for independent review. | `planning/dead-vocabulary/` | **claims nothing versioned.** Extends `make expression-census` behind `DECLARATIONS=1`; default output and authoring validation remain unchanged |
