...
| `semantic-collectors.md` | **draft 2026-08-22 — Wave C: the 20-row matrix adjudicated into 14 registered ids, 1 binding, and 5 assignments — zero new producers.** The three-way overload split pinned as measured (the broad detector is the **permanent hard negative**, and upstream `overloading()` is unconditionally `False`, so tag agreement is pinned structurally unavailable); `mate-proof@1` is a complete legal-tree proof with a node cap and three honest verdicts — deeper mating nets need a versioned proof tree, **never king-zone counts**; `promotion_geometry` binds to the D922-amended projection rather than duplicating it; the opening trio and Review rows are **assigned, not absorbed** ([[D925]]). All fourteen ids red-by-producer-absence verified by reading code; the Story mate→±1000 cp corruption is **cited as the Review successor's, never scored as this RFC's evidence**. A subtle mechanics find encoded: the shipped derivation-widening checker forces `declared_convention` + `input_abstained` on mixed-grounding derived rows — the conservative mapping is a named deviation | `planning/evidence-foundation-ux/` | **claims nothing versioned** — verified against all six registers; `at_commit` explicitly left to `learner-modules`; [[D904]] untouched. Four Discharges rows, P4/P6-clean: the literal-id module amendment (codex item 5), the Review lane, the opening trio, and [[D926]]'s authored witness debt |
