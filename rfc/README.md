...
| `learner-rating.md` | **cross-reviewed 2026-08-16 — not ready to accept, close.** D395 written in as **R15** with a byte-identity acceptance test (identical run at several ratings ⇒ byte-identical guard prompt, outcome panel, feedback events and voice packet). §5.4's tablebase adjudication **withdrawn** — it contradicted the RFC's own governing test. Six owner-facing questions; two should be answered before acceptance. **Register facts had gone stale under the draft** (`STORAGE_VERSION` 23, run schema 0.17, free lane 0.18) — *originally blocked by [[D395]]*: pin *a rating may select what is shown, never be an input to what is said about a move played*, before it lands | — | **claims no pack lane (0.28 stays free), no run schema, no shape-entry** — the rating is a projection over the existing event log. One **migration position** (`STORAGE_VERSION + 1`), landing **behind `teacher-surface`**, create-table only with **no backfill**. Glicko-2, argued from three repo numbers; 14 named refusals. **Flags that `docs/return-and-progression.md:48-49` currently forbids a learner rating** and asks for a scoped amendment at landing rather than making it |
...
| `feedback-delivery.md` | **draft — author round complete 2026-08-16; awaiting cross-review. OPEN QUESTION 1 IS CLOSED AND NO OWNER RULING IS NEEDED**, for three independent reasons, none of them the author's judgement: the owner **already ruled** on 2026-08-15 and refused the fork itself (*"why not fix them properly?"*, recorded as `claim-backing`'s exploration gate); **the premise is false in the tree** — `attachExplorerEvidence` makes no assignment into `pack` at all, the overreach guard raises `EVIDENCE_OVERREACH` unconditionally at `error`, and a `tablebase_result` record backs an authored sentence today; and `claim-backing` §6 **retained C6 and formally withdrew C6′ and C6″**, so the draft's own recommendation is reversed. **No class is structurally unbindable** — all four checked at the shipped checker. Day zero for delivery: **98 of 196 claims, 26,735 of 61,531 chars (50.0% / 43.4%)**; the residue is a sourcing wave (60 `corpus_observed` claims against **0 explorer records**) plus a handful of authoring fixes | — | **claims nothing versioned**, and takes **no migration position** — deliberately, so it does not become a fourth holder in [[D423]]'s collision. C7 turned out to have **already shipped** (`PackRecord.boundClaimIds` + `claimBackings`), so it now adds no field to `PackRecord` at all. Register facts at the author round were three-to-five lanes stale (0.22/0.15/20 against pack **0.27**, run **0.17**, `STORAGE_VERSION` **23**) |
...
| 23 | 22→23 | `opponent-contracts.md` | **implementing 2026-08-16** — stamp-only run schema 0.16→**0.17**; optional `OpponentSelection.orderingBasis`, historical selections remain absent and are never inferred. Body uses frozen literals |
