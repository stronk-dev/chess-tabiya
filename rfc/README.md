...
| `semantic-evidence-selection.md` | **draft 2026-08-21 — F2, opened after implemented F1 plus R1/R2/A3 and O2/O3.** Adds 22 exhaustive identity-preserving direct event projections, eleven registered denominator-bearing derived `avoided` projections, exact consumer eligibility, a parameterised local legal-alternative selector and the D668 payload seal. It leaves tactic/plan labels, production module defaults, learner-visible routing, persistence and all content migration out. D660 is corrected before drafting: the older 294× global-lift result remains diagnostic and is prohibited from product selection; the later R2 local selector is the transferable mechanism | `archive/evidence-contract-manifest.md`; structural/transition semantic residue | **claims no shared resource, schema lane or migration.** Extends the compiled manifest with independently versioned declarations; no production policy is selected until F5 |
