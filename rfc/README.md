...
| `pack-capability-contract.md` | **draft 2026-08-23 — F3, licensed by [[D995]]/[[D996]] (`o5-o6-handoff.md:100` reads `O6.1 + O6.2 approved → F3 may draft` verbatim), with its central fork ruled by [[D1058]]: the capability stamp lives IN THE PACK.** A pack declares **nothing** today — 24 keys under `additionalProperties: false` and **0 of 92** carrying any version — while its meaning is decided across **≥149 pack-facing vocabulary arms**, 188 manifest projections, **12 conventions whose semantics are frozen prose**, and 13 verdict producers of which **12 carry no identifier at all**. Capabilities are keyed on **evaluator identity, not JSON fields**, because two of the largest determinants of meaning are *ordering* semantics (`evaluateObjective`'s first-match-wins, the tempo cascade) that no field-keyed version can express. **The mechanism is a semantics digest** over each capability's source region plus its convention prose, so a meaning change without a version bump reddens CI — which is the [[D566]] test: that drift took `outpost` from 10 observations to **0 of 643** with no schema change, no digest movement and no version increment, and criterion 13 fixtures exactly it. Also fixed: the version becomes **data** (`evidence-manifest-check.ts:70` pins all 67 semantic events at `@1` inside an assertion literal), one spelling replaces three, and a `refused` disposition must carry `ruledBy` — [[D1045]]'s asymmetry is not inherited. **Gate F clause 1 goes from two-deep to three**, stated plainly rather than buried: the owner took binding integrity over the content hold's lift date | F3 in `rfc-graph.md:70`; F1's manifest; `graduation-clearance` §6.5's planner precedent | claims **pack-schema lane 0.30** — `requires` (new, required) + `$defs/capabilityRequirement`; the stamp is inside `digestDrillPack`, so **all 92 pack digests churn** and every ledger re-stamps (Discharge D3, held behind [[D949]]'s graduation arm). **No migration position** — packs are files, not rows |
