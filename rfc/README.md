...
| `teacher-surface.md` | **cross-reviewed 2026-08-16: ready on the mechanics, OWNER-BLOCKED** on one question — does the review surface work without Maia and corpus? | — | holds the next migration position after board annotation's landed **22**; its eventual number is assigned at landing. Four tables, `run_grants.expires_at`, `live_sessions.classroom_id`; no run- or pack-schema change. No new token scope, fourth `RunRole`, or session kind. Fixes D80 as a pure narrowing |
