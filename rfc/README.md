...
| `semantic-evidence-selection.md` | **implementing 2026-08-21 after acceptance, two buildability reviews and D684's retained external fixture repair.** First unit: compiled contract and falsifying fixtures. Adds 22 exhaustive identity-preserving direct event projections, eleven registered denominator-bearing derived `avoided` projections, exact consumer eligibility, a parameterised local legal-alternative selector and the D668 payload seal. It leaves tactic/plan labels, production module defaults, learner-visible routing, persistence and all content migration out | `archive/evidence-contract-manifest.md`; structural/transition semantic residue | **claims no shared resource, schema lane or migration.** Extends the compiled manifest with independently versioned declarations; no production policy is selected until F5 |
