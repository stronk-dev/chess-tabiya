...
| `graduation-clearance.md` | **accepted 2026-08-16 (second author round) — and the acceptance test has changed.** It was accepted once already, on the wrong test: its four author-call open questions were closed, and the implementer returned it hours later with four blockers **none of which was an open question** ([[D473]]). All four were *"who calls this, with what, where"*. **The test now applied is buildability** — every obligation resolving to a named symbol, command or home — and it is verified: **D464** gains `clearance.recordKind`, its enum transcribed from the shipped `EVIDENCE_KINDS` with a criterion asserting set-equality; **D465** gains an eighth kind `referent_removed`, after walking all 30 resolved entries and finding **29 resolve and 1 does not**, so Stage B is stated as **29 mechanical + 1 by hand** rather than a 30-entry migration; **D466** gains a writer, `make graduation-clear`, modelled on the shipped `verifyDraft`, with a mandatory `packDigest` re-stamp; **D467** is split into **two rules that cannot be one** — a runtime *shape* rule with a zero-filesystem budget and an authoring *admission* rule that needs `.git`. **The round also found the defect already ships as [[D468]]** — a boot failure, not a hypothetical. Correction landed en route: §1.2 named the **wrong join** for two review rounds ([[D471]]) | — | keeps its **pack schema 0.28** claim while returned. No migration, run-schema, or shape-entry claim |
