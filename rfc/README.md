...
| `theming.md` | **draft 2026-08-22 — the [[D839]]/[[D840]] lane, drafted on the owner's same-day double ruling ([[D976]]): default FOLLOWS THE DEVICE with manual override, and the identity is a CATALOG OF KNOWN SCHEMES — Tokyo Night first, its palette inherited VERBATIM from the owner's own registry** (`~/frameworks/mistserver` LSP `themes.js`; the set-of-themes pattern read from all three of the owner's reference implementations, not reinvented). A **total token contract** (16 color tokens + shadow; every theme is a `Record<ThemeToken,string>` — a half-theme is a compile error AND a runtime criterion) over the shipped 9-token layer; v1 catalog: `paper` (the warm light, byte-identical baseline), `tokyo-night` (dark+light), `warm-dark` (the FullDark canvas candidate, committed here as its durable record). **The load-bearing move is the skin/paint split**: `chessground.brown.css` fuses square color with ALL evidence paint (last-move, dests, check, brushes) so a naive board swap silently swaps evidence paint — board themes become skin-only files over one shared token-driven `interaction-paint.css`, with the four `MARK_BRUSHES` identities + lighting blue held pairwise-distinguishable (ΔE ≥ 20) per theme and the [[D493]] boundary pinned BOTH directions (criterion 10: no theme value reaches `compileAssistance`, no compiled output selects a theme). Ships the [[D840]] animation pref (None/Fast/Normal, reduced-motion forces None); felt-quality verification stays the owner's (D840 flips on their session, not on green tests). Criterion 1 is **red at HEAD** on the two phantom tokens (`--surface`/`--panel-soft`, used but defined nowhere — proposed D977). Three owner opens, all non-blocking candidates | `planning/theming/` | **claims nothing versioned** — client-only CSS + `tabiya.theme.v1` localStorage (global, version inside the value; deliberately NOT the per-context assistance/workflow grammars); no schema, no migration, no lane; board/piece assets are bundled CSS with a license-manifest criterion (cburnett CC BY-SA, Tokyo Night MIT, chessground GPL-3.0+ — each verified `[V]` at implementation or the asset does not ship) |
