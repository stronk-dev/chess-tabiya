...
| `tactical-collectors.md` | **implementing 2026-08-22 — first 20/30 projections landed; the final ten are held for independent review of the D829–D835/D931 in-place amendment.** The amendment makes the returned meanings executable: cited space convention; directed pawn-support graph; undefined rook cutoff deleted; immediate trade anchors; mover-relative loose-piece delta; discovered latency→execution binding; total promotion geometry with typed field absence. All families remain research/inspector-only. Module promotion, presets, authored vocabulary and opening identity stay outside this landing | `planning/tactical-collectors/plan.md`, `planning/evidence-foundation-ux/tactical-collectors-review.md`, `planning/evidence-foundation-ux/collector-rfc-joint-review.md`, `design/research/wave-a-contract-closure.md` | **claims nothing versioned**; authorable vocabulary remains separate |
| `bot-policy.md` | **draft 2026-08-22 — F8: the seven-layer stack where a persona is a COMPOSITION and honesty is structural.** A hidden error guard is **unrepresentable** — the disclosure string is a compile-time requirement; the **label rule is normative**: *a stated Elo is a measured claim with its measurement cited, or it is not stated* (no vendor in the field survives it); forcing/quiet traits ship as **permanent negative fixtures**; salience-shaped error is **folklore until D815 passes and says so in the text**; the [[D843]] wall (learner analysis never feeds personas) is a **closed input vocabulary = compile error**, plus a census criterion. The sampler reconstructs production Maia with R11's captured-production conformance fixture; Stage A ships three personas on position-level features with the two-stage collector seam as Discharges D1; the pick record ([[D677]]'s no-prior-art mechanism) joins the existing candidates array and buys the tournament envelope for free. Weakened-Stockfish stays refused **at mechanism level** and kept only as the blind packet's negative control | `planning/platform-alignment/bot-policy/` | claims **run-schema lane 0.18** (`OpponentSelection.policy`) + **one migration position behind `longitudinal-store`** — both verified free at HEAD; policy definitions are a **catalog-local registry, not a table**, argued on four grounds with the user-composition exit named ([[D936]]) |
| `move-quality-grades.md` | **draft 2026-08-22 — the [[D879]] executable, and its kill exhibit is the argument**: under the shipped mate→cp coercion, a missed mate-in-2 from +8 measures a 2.54-point drop — **below every threshold** — so the coercion makes exactly the moves a grade family exists for invisible; F-MATE-NEG stands as the permanent hard negative. Win%-drop basis with the scalachess logistic pinned and the raw-cp refutation shown in four rows (+500→+300 and +100→−100 are the same 200 cp and are NOT the same mistake); typed mate classes; two cited ladders with **every constant carrying `{value, source, pinnedAt}` as lint-checked fields**; chess.com's expected-points basis refused as unpinnable AND rating-conditioned. **Best-move refused by the compiler, not by promise** — a grade carrying a move fails `EVIDENCE_DERIVATION_WIDENS`. Zero `voice.ts` changes; grades never persist (criterion 10 is a labeled regression guard, vacuously green). Until D917's Story repair lands, Story and the grades will **visibly disagree about the same moment — named as D917's bug surfacing, so no reviewer weakens the grade to hide it** | `planning/evidence-foundation-ux/` | **claims nothing versioned** — catalogue-local ids, no migration (nothing persists); WDL basis deferred on [[D927]]; consumed by `postcommit_nudge`/`review_map` only, compiling the two ◇ rows |
...
| `play-composition.md` | **accepted 2026-08-22 by claude as register owner, on the buildability test, after cross-review (corrections applied in place).** The review's catches: a **live board-resize hiding inside the stage** — the text-move disclosure is a grid row of `.board-shell`, so opening it shrinks the board; acceptance **state 16 starts red at HEAD** on it ([[D923]]); three unbarred layout children found by the closed-children audit; **naive `{#key}` removal is unsafe** (the key's second term is the branch-group capture-reset — A9 carries the replacement mechanism); four leak destinations lacked contract permission ([[D924]], discharged by the learner-modules Appendix-B amendment, inspector 34→40). Geometry proven by a seven-viewport numeric exhibit — all 8-divisible, all ≥192, no overflow. 7×**16** states, **112** screenshots, post-gesture throughout | `planning/evidence-foundation-ux/` | **claims nothing versioned** — web code/tests/docs only; two open questions flagged for cross-review (the 1024 tablet breakpoint and the compact-tab successor are **named opens, not measurements**) |
