...
| `teacher-surface.md` | draft (awaiting cross-review) | — | holds the next migration position after board annotation's landed **22**; its eventual number is assigned at landing. Four tables, `run_grants.expires_at`, `live_sessions.classroom_id`; no run- or pack-schema change. No new token scope, fourth `RunRole`, or session kind. Fixes D80 as a pure narrowing |
| `opponent-contracts.md` | draft (awaiting cross-review) | — | claims **run schema 0.17** (`OpponentSelection.orderingBasis`), a **stamp-only migration at position `STORAGE_VERSION + 1`**, **pack 0.28** (five `FORMAT_DISPOSITIONS` rows + a totality gate — flagged by its author as its weakest claim, to be *released* rather than rebased if cross-review disagrees), and an unversioned `/capabilities` change. Names the **third direction** of the declared-vs-executable law: executed, load-bearing, never stated |
| `measurement-records.md` | draft (awaiting cross-review) | — | claims **shape-entry schema 0.4** only — one optional `measurements` property + three `$defs`. **No pack lane (0.28 stays free), no run schema, no migration.** Splits the record into a normative half the gate may fail on (`textSha256` + span agreement) and a diagnostic half it may never fail on. Refuted `{measuredAt, packs, positions}` on the real readings: 4 true defects **and 8 false alarms** |
...
| `pack-graduation.md` | **accepted 2026-08-16** | — | Structures `provenance.graduationBlockers` as `blocking`/`resolved`/`accepted`; graduation is a **move**, not a copy. Claims pack **0.27**. Wires the sourcing gate **strictly over `content/packs/` at zero cost today** and as a **ratchet** over `content/drafts/`; the strict flip has a named owner and a mechanical trigger rather than a wave |
...
| 0.27 | `pack-graduation.md` | claimed 2026-08-16 — `provenance.graduationBlockers` entries become objects with `blocking`/`resolved`/`accepted` state, and `provenance` closes `additionalProperties` on its five attested keys. **Deliberately took 0.27 over the then-free 0.26** rather than race `claim-backing`, which is its own hard prerequisite; rebasing down is free since the `$id` is in no document |
