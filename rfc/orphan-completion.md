...
- **Exploration gate:** opened by owner ruling 2026-08-12 (`rfc/README.md:75-82`); breadth
  ...
  2026-08-14 polish wave by the owner ("all four scheduled into polish",
  `planning/roadmap-to-done.md:17,19`; ledger rows `design/BACKLOG.md:222-224`)
...
- **Wave claim:** second in the 2026-08-14 (second) three-draft wave (`rfc/README.md:23-26`).
  ...
  no run-schema version, and no new table** (voice output stays ephemeral and distillation
  proposals are response-derived, so nothing persisted changes shape). Its shared implementation surfaces, named for the sibling drafts:
...
(`planning/traceability-forward.md:155-169`, `design/BACKLOG.md:222-224`). All three are thin
...
`POST /runs/:id/compare` (branch ids in the request body; the action sits in the
POST section of the run-route dispatch, `apps/server/src/rest.ts:1257-1263`): the client already holds both
...
- Evaluation pivots reuse the documented story convention verbatim: mate maps to the ±1000 cp
  ...
  (`docs/game-import-and-story.md:73-77`). These are pinned parameters with one revision point,
  not per-surface facts: the convention ships as literals in the story renderer
  (`packages/runtime/src/story.ts:48-51,84`), and this RFC's implementation extracts them as
  shared named constants consumed by both the story and the narrative builders, so a future
  change to the convention changes every surface and its documentation together or not at all.
  Arithmetic over recorded evidence, never a move grade.
- Branches are named in every template by **fork offset and forking SAN** — facts from
  `columns`/`rows`. Learner-entered branch `label` and `intent` text (free strings accepted at
  fork time, `apps/server/src/rest.ts:1244-1249`) never enters a narrative template. The client
  compare screen may keep showing labels as column headers, as it already does
  (`CompareView.svelte:49`) — that is the viewer's own text on the viewer's own screen — but the
  narrative, and therefore every packet built from it (§1c), contains no learner free text.
...
Because §1b bans learner-entered `label`/`intent` text from every template, the `compare`
packet upholds the adoption-wave-1 wire pin — persona prompt, deterministic sentences, and
scope only, no learner-supplied text and no learner/run/session/client identifier
(`docs/adoption-wave-1.md:57-64`) — which no earlier scope has breached and this one may not
be the first to breach. Provider behavior is the existing contract byte-for-byte: packet-bound phrasing only,
...
4. `checkpoints`: the definitions of checkpoints that actually fired, copied from the
   digest-resolved source pack; unreached checkpoints are dropped and listed in `dropped`.
   Copied triggers are **remapped, not copied blind**, because distilled spine ids derive from
   run node ids (item 2) and a source pack's spine ids would dangle: `atPly`, `fenPredicate`,
   `materialBalance`, and structural triggers copy verbatim (they were valid against this same
   root); an `atSpineNode` trigger is rewritten to the distilled spine id of the node its
   `checkpoint.reached` event records (`packages/runtime/src/types.ts:154-157`); an
   `atAuthoredBoundary` trigger, and any timing window whose spine anchors cannot be remapped,
   cannot survive without the source pack's authored boundary structure — its checkpoint is
   dropped and listed, with the reason. A position-sourced run — or a run whose surviving
   checkpoint set is empty, whether because none fired or because every fired one was
   boundary-anchored (the schema refuses `checkpoints: []`, `minItems: 1`) — gets exactly one
   mechanical `atPly` checkpoint at the selected branch's deepest learner ply with a summary
   stating it is mechanical; the substitution is named in `graduationBlockers`. That mechanical
   trigger clears both root gates by construction: the branch has at least one learner ply, so
   `atPly >= 1`, which can be neither `CHECKPOINT_UNREACHABLE_AT_ROOT` (fires only on
   `atPly === 0`) nor `CHECKPOINT_TRUE_AT_ROOT` (an `atPly >= 1` trigger cannot match the
   root), `apps/server/src/pack-validation.ts:620-639`. A selected branch with zero learner
   plies refuses with `IMPORT_INVALID`.
...
7. Remaining required scalars: `id`/`title` from the request; `version` `"0.1.0"`. `mode` and
   ...
   exactly one `atAuthoredBoundary` checkpoint (`pack-validation.ts:411,424-427`), and
   `run_trajectory` requires `legs` with entry checkpoints
   (`pack-validation.ts:323-332`; `schemas/drill_pack.schema.json:100-104`) — authored
   structure a run does not carry — so `line`-with-`follow_theory` and `trajectory` sources
   distill as mode `"outcome"` with the mechanical objective, the downgrade named in
   `graduationBlockers`; position-sourced runs get `"outcome"` / `"play_until_checkpoint"`
   directly. `objective` is always exactly `{ type, summary }` with the mechanical placeholder
   summary the archived extract specified (`rfc/archive/pack-studio.md:1545-1550`);
   `objective.grading` and `successConditions` are **never** copied — grading carries chess
   claims and retrieval provenance bound to the source pack's own ledger, and success
   conditions are authored judgment, both the author's to re-add in the studio.
   `feedbackPolicy` = `run.feedbackPolicy`, with the run-only `attempt_end` substituted by
   `delayed_checkpoint` (the pack enum admits only
   `delayed_checkpoint | segment_end | immediate_guard`,
   `schemas/drill_pack.schema.json:56-58`) and the substitution named in
   `graduationBlockers`.
...
- **Structures met but never drilled** — `shapeFirings`
  ...
  (`ShapeFiring.entryId`, the well-defined join key against `pack.shapes` entry ids),
  subtract every entry referenced (`pack.shapes`, `docs/shape-library.md:31-33`) by any pack
  with at least one **countable** attempt row for the caller — countable, because an attempt
  is any preserved branch and an empty fork is recorded but not counted
  (`docs/return-and-progression.md:3-14`), and the sentence must state only what the rows
  record. Item: `{ kind: "shape_encounter", shapeId,
...
preserved runs and have no countable attempt recorded in any pack that names it." Forbidden by
...
5. **Distillation properties (server).** Restating the archived acceptance 10/10a
   ...
   fired checkpoint yields `checkpoints.length === 1` with the substitution named; a fired
   `atSpineNode` checkpoint is remapped to the distilled spine id of its `checkpoint.reached`
   node and a fired boundary-anchored checkpoint is dropped with its reason listed; a
   `follow_theory` or `trajectory` source distills as mode `outcome` with the mechanical
   objective and the downgrade named; a zero-
...
6. **Recommender browser test.** After the existing repertoire import-and-scan flow
   ...
   encounter recommendation whose link lists packs naming the entry; recording a countable
   attempt on such a pack removes it.
...
- 2026-08-14 (adversarial review, fixed in place): wave claim now states run-schema
  neutrality explicitly; `/runs/:id/compare` corrected to POST; the ±1000/150 story
  convention is pinned to shared named constants extracted from
  `packages/runtime/src/story.ts:48-51,84` (one revision point, not a smuggled per-surface
  fact); narrative templates are pinned to name branches by fork offset + forking SAN,
  banning learner-entered `label`/`intent` from every template and therefore from the
  `compare` voice packet (adoption-wave-1 wire pin preserved); distillation §2b.4 gains the
  checkpoint-trigger remap rule (`atSpineNode` → fired node's distilled spine id,
  boundary-anchored triggers dropped and listed) plus the proof the mechanical fallback
  clears `CHECKPOINT_UNREACHABLE_AT_ROOT`/`CHECKPOINT_TRUE_AT_ROOT`; §2b.7 pins the
  objective to `{ type, summary }` with mechanical summary, never copies
  grading/successConditions, and downgrades `follow_theory`/`trajectory` sources whose
  authored structure a run cannot carry; acceptance 5 extended to match; the recommender's
