...
- **Status:** implementing 2026-09-24 at the owner's direction to implement ready RFCs without
  further review rounds. **Landed:** the §1 compiled registry (48 declarations: the 39 reviewed
  initial members generated byte-for-byte from `initial-declarations.json`, plus nine authored
  declarations bound to the code that computes them), the append-only semantic history and its
  check, the §1.1 `semantic-conventions` register (48 members), mechanical direct convention closure
  for every mintable projection, and the §4 value-level `ConventionReceipt` sealed by the sole mint
  with every value ([[D1921]]); the recorded semantic path now carries that receipt ([[D1929]]).
  **Remaining:** §2 typed instance extractors, adapter key equality and the total `not_applicable`
  audit; §4 rendered disclosure and provider limitation assembly; §5 presentation; §6.1 run-schema
  0.24 attested persistence. Receipt: `planning/semantic-convention-provenance/implementation-2026-09-24.md`.
  Previously: draft — amended 2026-08-28 to reconcile the [[D2013]]–[[D2019]] process return
...
- **Depends on:** implemented `rfc/archive/evidence-contract-manifest.md`; implemented
  ...
  `rfc/archive/semantic-evidence-selection.md`; implemented process row
  `rfc/semantic-convention-register.md`
...
The run-schema claim is required now by the durable Review/history guarantee in §6.1 and stays
live until §6.1 lands. The `semantic-conventions` membership claim was declared in the process
landing of `semantic-convention-register.md` (the catalogue row over the present, initially empty
tuple) and consumed by this RFC's landing of all 48 members, so the block no longer carries it: the
members are Landed rows of the README register, set-equal to `SEMANTIC_CONVENTION_MEMBERS` and to the
...
|---|---|---|
| [[D1921]] | **closed 2026-09-24 in production**: the sole mint seals a value-level `ConventionReceipt` (direct ∪ exact sealed-input closures, canonical used-input member, input value-digest multiset) atomically with every value; nothing attaches ancestry after freeze | `packages/runtime/src/evidence-factories.ts#mint`, `evidence-convention-closure.ts` |
...
| [[D1925]] | **closed 2026-09-24**: `evidence-convention-history.jsonl` plus `make semantic-convention-history-check` (working tree, index and first parent); the process register landed as a catalogue row | `tools/semantic-convention-history-check.mjs` |
...
- 2026-09-24: implementing (claude, worktree `agent-ad09667cb734eccf8`) at the owner's direction.
  Landed §1 (registry, generator, source check, append-only history and its check), §1.1 (the
  `semantic-conventions` register, 48 members, over the catalogue row `semantic-convention-register.md`
  introduced empty in the preceding commit), direct convention closure for every mintable projection
  and the §4 value-level receipt; this discharges `evidence-value-authority.md` D1 and closes
  [[D1921]]/[[D1929]] for `recorded-semantic-path.md`. Corrections made while implementing, none
  widening authority: (a) the register is one generic `members` row (`defence-duty@1` is member
  `defence_duty_v1`) because the implemented bootstrap has no lineage reader; lineage is enforced by
  `compileConventionRegistry` and the history check instead. (b) The runtime package is
  browser-buildable and cannot read the JSONL history, so "the compiler compares the registry with
  the history" is executed by `make semantic-convention-history-check` and
  `evidence-conventions.test.ts`, not at module load. (c) Nine declarations beyond the 39 are
  authored directly, as §1.2 allows for future declarations: `phase-bands@1`,
  `endgame-material-census@1` and `named-structure-catalogue@1` (the meanings
  `evidence-value-authority.md` §3.3–§3.4 shipped under those ids, the last given an identity like
  the sixteen assigned ones) and the six cited endgame setup/method conventions, which move here from
  their interim in-code registration; each is bound byte-for-byte by a test to the code that computes
  it, and the history attributes their meaning to `evidence-value-authority.md`. (d) §2's per-projection
  `conventions.direct` field is derived mechanically (witnesses, successor retention, registered
  `id@version` tokens in the catalogue text, one declared inheritance row) in
  `evidence-convention-closure.ts` instead of being hand-written into 200+ declarations; 70 of 223
  mintable projections carry refs. (e) §4's `member` is the canonical identity of the exact sealed
  inputs actually used. Resolving it to one *declared* manifest derivation member is not enforceable
  yet: a measured run over the software suite found production factories legitimately using input
  subsets of the manifest's union members (`derived.story.rank@1`, `derived.opponent.candidate_feature_vector@1`,
  the recorded readings over sourcing-ledger inputs). That validation stays with the remaining §2
  work rather than being faked. (f) Setup and method values carry only the instance convention they
  were computed under; their catalogue text lists all alternatives and is excluded from the direct
  closure so unused alternatives never enter a receipt.
