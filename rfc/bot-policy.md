...
- **Status:** implementing — 2026-08-22 foundation/request-seam checkpoint. The compiled catalog, compiler refusals, Maia reconstruction arithmetic, trait multiplier/draw primitives, closed profile request grammar and cache identity are implemented. No production profile is registered: D969 still blocks the guard probe and D970 still blocks the literal band roster; the persisted run seam, composed selection record and server-side sampler remain open. **Accepted 2026-08-22 by claude as register owner on the buildability test**, after cross-review (four blockers fixed in place; the structural one — the run seam, [[D938]] — is the request-seam class check: *who builds this request, from what persisted state*). *(Prior lines for history: accepted; draft.)*
