...
- **Status:** implementing — **implementation landed 2026-09-24 at the owner's direction** (implement directly, no review round; consolidation, review and the `rfc/README.md` Active-row status transition come later, so the token stays `draft` for status parity). The mechanism ships over the landed store: `STYLE_METRICS` + `make style-registry-check`, the card grammar and abstention, `STYLE_REFUSED_TERMS`, `assertTierRuleGrounded`, the reference wall, the catalogue/habit-card type split, the one-sealed-card paraphrase gate, a private read-time profile through `createApplication` (`/learner-profile*`) and a Svelte 5 `/profile` surface linked from Rating and Learn. Nine metrics compute at read time; opening surprisal (no pinned reference) and the three clock metrics (recorded-clocks unlanded) abstain by name. **Every card stays dark until its own floor clears**, and §11.3's transfer arm (D6) remains unmeasured and is disclosed on the surface. Receipt: `planning/style/implementation-2026-09-24.md`. Draft history follows. **Draft (2026-08-23):** **The licence is honest and partial, and that is stated rather than finessed.** [[D1093]]'s drafting mandate enumerates exactly three lanes ([[D1031]] variants, [[D1041]] time controls, [[D1060]] famous games) and **this is not one of them**; worse, this lane carries a *contrary* routing note — [[D1055]]: *"foundation amendments + measurement precede D552 surface RFC"* (`design/BACKLOG.md:392`). What licenses the draft is that the owner has twice instructed full breadth and depth and **explicitly rejected the scope cut** this lane previously received ([[D1232]], [[D1230]]), and [[D552]] is their verbatim ask. **What no licence can supply is the measurement**: this is the one lane where the owner's signature is not the binding constraint — §11's blockers are named, cited, and several are ~8 weeks of somebody's real play. Acceptance is therefore split: the mechanism can be accepted; **no card renders until its own row clears its own floor.**
...
> `make style-registry-check` asserts `STYLE_METRICS` is **set-equal by metric id, with feature id and floor compared per row,** to the rows of `tools/r21-style-feedback-contract/registry.ts` whose `persistentFloors` value is non-null, and **fails on an omitted, added or re-floored metric**.
...
1. **The registry is derived, never copied.** `make style-registry-check` passes, asserting `STYLE_METRICS` set-equal by metric id — with feature id and floor compared per row — to R21's non-null-floor rows. *Wrong implementation that passes without this:* one that hard-codes twelve rows and silently drifts when R12 is re-measured. The count is a tripwire, not the assertion.
...
14. **The store precondition is asserted, not assumed.** A composition-time check that fails while `learner_observations` is absent, with the message naming D973/D1011 and migration 26. *(Green since migration 26 landed; the absent arm is a fixture.)*
...
| D3 | [[D1052]]'s one-line SHA correction, unblocking metrics 1–2 | claude | the `runtime-opening-identity` amendment commit | 2026-09-24 — moot: `runtime-opening-identity` compiles and ships (`44637013`); metric 2 computes, metric 1 still needs its reference population |
...
- 2026-09-24 — **implemented at the owner's direction; text defects fixed inline, every other finding became a test.** Corrections:
  ...
  3. **§8 drill-down on an abstaining card leaked the numerator.** Listing the occurrences behind a card below its floor discloses k. An abstaining card's drill-down lists the games measured so far (its denominator); occurrences open only once the card measures.
  4. **§2.2 left "game" undefined for rewound runs.** A measured game is a run that began from the standard starting position with ≥1 store-attributed `played` decision; only its first line (`branches[0]`) counts. Decisions come from the store's own `normativeDecisions` at the exact completed cut and are cross-checked against its `played` denominators.
  5. **§2.3 / D1–D2 sequencing.** The five atoms and the castling eligibility projection are implemented as read-time rules predicates in `packages/runtime/src/style-atoms.ts` (transcribed from D1062, `chessops.castlingSide`), not as collector registrations or store rows. D1 and D2 stay open; when they land the card source moves to store rows under the same feature ids. Nothing is persisted, so no migration is claimed.
  6. **§11.1 is satisfied, and criterion 14 now guards it.** Migration 26 landed; `assertLongitudinalStorePresent` refuses to compose the profile without the three observation tables and names D973/D1011 and migration 26.
  7. **§11.2 metric 2 is unblocked; metric 1 is not.** `runtime-opening-identity` compiles and ships, so D3's SHA fix is moot and opening-family entropy computes; opening surprisal still needs a pinned, versioned reference population and abstains as `reference_population_unpinned`.
  8. **§8 sharing needs no stored consent.** A share is an explicit per-card request carrying `consent: true` that returns text (sentence and numbers only); nothing is published or persisted, so no table or migration position is needed. A request without consent, or for an abstaining card, fails.
  9. **§10.1** ships as the admission and output check (`admitStyleParaphrase`, `styleParaphraseOutputCheck`); no paraphrase route ships, so the deterministic card sentence is the only prose.
  Open question 2 is applied as recommended (`played` only; imported games' moves never count as "your play"); Open questions 1 and 3 stay the owner's — the surface is a private profile, never a gate or a progression input.
