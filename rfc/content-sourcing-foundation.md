...
| A `/create` UI, draft store, or pack write endpoint | B6-1. `apps/server/src/rest.ts:375,378` expose `GET /packs` and `GET /packs/:id` and nothing else on that prefix — no POST/PUT/PATCH/DELETE. This RFC's output is files on disk, consumed by `make pack-check` today and by B6-1 later |
...
of this territory died on unverified capability claims. `[V]` throughout.
...
| `outcome.reached` has no producer | declared `packages/runtime/src/types.ts:157`, projection no-op `packages/runtime/src/events.ts:134`; no emitter |
| Only `reach_checkpoint` **success conditions** execute | `pack-validation.ts:159-178` rejects anything else; `apps/server/src/pack-orchestrator.ts:88-100` maps only that kind to a predicate |
...
| Checkpoints are evaluated after **both** learner moves and opponent plies | `apps/server/src/service.ts:195` and `:214` both call `orchestratePackMove` |
...
| **`start.side` is the learner's colour, not the FEN's side to move** | `apps/web/src/lib/session-controller.ts:345` moves the opponent whenever `boardModel(node.fen, packStartSide(pack)).turnColor !== packStartSide(pack)`. All three drafts exercise the difference: `carlsbad-minority-attack.json` has a `b`-to-move FEN with `side: "white"`; `rook-4v3-same-side.json` has a `w`-to-move FEN with `side: "black"` |
| **`start.side` is schema-optional but client-required** | `schemas/drill_pack.schema.json:117` does not require it; `apps/web/src/lib/screen-model.ts:54-59` throws `TypeError` when it is absent, and `apps/web/src/lib/DrillScreen.svelte:108` calls it unconditionally. Every emitter must write it |
| **`theory_strict` silently degrades to `human_common`** when the pack has no spine, or no spine child at the current position | `apps/server/src/opponent-selector.ts:454-457`, reached through `spineChildren` (`:337-347`) |
...
| Stockfish evidence ships with `multiPv: 1` and a 100 ms default movetime | `apps/server/src/strong-engine.ts:10-15`; payload kinds `eval`/`wdl`/`bestline`, source `engine_validated` (`apps/server/src/evidence-queue.ts:324,339,360`) |
| The strong-engine profile is overridable | `strong-engine.ts:23-31` (`resolveStrongEngineProfile`) |
...
- `provenance.licence` and `provenance.attribution[]` — the share-alike encoding required by
  ...
  "entries": [
    {
      ...
      "bytes": 123456,
      "licence": {
        "spdx": "CC0-1.0",                      // closed set; see the table below
        "attributionRequired": false,
        "shareAlike": false,
        "noticeText": null,                     // string, required when attributionRequired is true
        "rationale": null                       // string, required when spdx is "unlicensed-data"
      }
    }
  ]
}
...
`design/research/theory-sourcing.md:134-143` (TWIC, PGN Mentor, the `ecochessopeningcodes`
compilation, bulk Lichess studies, `calebjcourtney/db.sqlite3`) are a hard-coded deny list
matched on hostname *and* on the `sourceId` vocabulary; a fetch against one fails with
`SOURCE_DENIED` naming the dossier line.
...
| `CC-BY-SA-4.0` | attribution **and** share-alike on derivatives | `noticeText` mandatory in the manifest; the full §2 encoding in the pack |
| `unlicensed-data` | none, and the *reason* must be stated | `rationale` mandatory, carried verbatim into `provenance.sources[]` (B6b §Licence; B6c §Licence) |
...
- **Concurrency 1, globally**, across all pipelines and hosts.
  `design/research/theory-sourcing.md:37-38` quotes the Lichess API spec: "Only make one
  request at a time."
...
- **Disk cache is the default read path.** Key = SHA-256 of the canonical request
  (method, host, path, sorted query). Entries store body, status, headers, and
  `retrievedAt` **written once, at first fetch, and never rewritten on a cache hit**.
...
`design/02-product-shape.md:41-50`, logged at `planning/exploration/log.md:901-903`):
authored pack prose is `CC-BY-SA-4.0`. Share-alike sources are therefore **accepted**, not
refused. What replaces the refusal is an *encoding* obligation, because
`design/research/theory-sourcing.md:61-63` forbids mixing postures per-paragraph and warns
that "provenance tracking will not survive edits" unless it is machine-checked.
...
- A pack whose prose derives from a share-alike source declares `provenance.licence:
  "CC-BY-SA-4.0"` and one `provenance.attribution[]` entry per contributing source, each
  carrying all five fields. `attribution[]` entries are sorted by `sourceId` then `url` so
...
- **`LICENCE_MIXED`** — a single pack draws prose from postures
  `design/research/theory-sourcing.md:61-63` declares incompatible (concretely: any pack
  declaring `provenance.licence` other than `CC-BY-SA-4.0` while carrying a share-alike
  attribution entry, or a pack carrying share-alike attribution with no `provenance.licence`
  at all).
- **Packs with no borrowed prose declare no `licence` and no `attribution` and are
  unaffected.** All three hand-authored drafts are in this class today —
  `content/drafts/{anti-caro-advance,carlsbad-minority-attack,rook-4v3-same-side}.json` carry
  original prose only. **Hand-authored packs must not become unpublishable by tooling.**
- "Contributed prose" is determined by the manifest, not by inference: an emitter records
  ...
  is in that class: ECO codes and move sequences are facts, not prose.
...
| `objective.successConditions` | `[{ "kind": "reach_checkpoint", "checkpointId": "line-end" }]` | the only executable condition (`pack-validation.ts:159-178`) |
...
| `opponentPolicy` | `{ "mode": "theory_strict" }` | `capabilities.ts:10-14`. Legitimate here **because the pack has a spine**; without one it would silently degrade (`opponent-selector.ts:454-457`) |
| `feedbackPolicy` | `"delayed_checkpoint"` | `pack-validation.ts:103-122` |
| `provenance` | `reviewStatus: "draft"`, one CC0 source string, `reviewers: []`, no `licence`/`attribution` (no borrowed prose), `graduationBlockers` listing every unwritten assertion category | `schema:458-475` |
...
2. asserts every `sources.json` entry has a `licence` with an allowed `spdx`, the
   conditionally-required `noticeText`/`rationale`, and that no entry's host or `sourceId` is
   on the deny list; fails `MANIFEST_EMPTY` when there are no entries;
3. asserts the §2 attribution encoding: `ATTRIBUTION_MISSING`, `LICENCE_MIXED`, and the
...
2. **`--learner-side` is mandatory.** Omitting it exits non-zero with a message naming
   ...
   `packStartSide` (`apps/web/src/lib/screen-model.ts:54-59`) accepts it.
...
4. `lichess-chess-openings` → `spdx: "CC0-1.0"`, `attributionRequired: false`; the emitted
   `provenance.sources[]` contains the CC0 statement and the pinned commit SHA; no
   `provenance.licence` and no `provenance.attribution` are written.
...
6. **Share-alike is enforced, not refused.** A manifest entry with `CC-BY-SA-4.0` and a
   `noticeText` emits successfully; the same entry recorded as contributing prose to a pack
   with no matching `provenance.attribution[]` entry fails `ATTRIBUTION_MISSING`; a pack
   carrying share-alike attribution with no `provenance.licence` fails `LICENCE_MIXED`; and
   all three of `content/drafts/{anti-caro-advance,carlsbad-minority-attack,rook-4v3-same-side}.json`,
   which borrow no prose, pass audit mode with no `licence` field.
7. **A share-alike entry that supplied only geometry does not trigger attribution.** A
...
8. **Offline reproducibility is byte-exact.** Every emitter runs under `--offline` in CI
   against committed fixtures; two consecutive runs produce byte-identical `pack.json`,
   `evidence.json` and `sources.json`, asserted by SHA-256 over each file. A third run with
   the system clock advanced by 48 hours produces the same three digests.
9. **No wall-clock field exists.** A test greps every emitted artifact for `generatedAt` and
   fails if it appears; `sourcedAt` equals the maximum `retrievedAt` of the consumed manifest
   entries; mutating `sourcedAt` by one second fails `EVIDENCE_TIMESTAMP_DERIVED`.
10. **A candidate with an empty manifest is refused** with `MANIFEST_EMPTY`.
11. **No emitter writes a key the schema forbids.** A property test emits one candidate and
...
12. **Evidence never enters the served projection.** `GET /packs/:id` for a promoted
...
13. **Digest stability under evidence refresh.** Rewriting `evidence.json` with new records
    leaves `digestDrillPack(pack)` byte-identical.
14. **Sidecars are not loadable as packs.** A test places `evidence.json` in a temporary
...
15. **`theory_strict` is only emitted with a spine.** A test asserts every P1 candidate has a
    non-empty `spine`, and a second test constructs a spine-less pack with
    `mode: "theory_strict"`, runs one opponent selection, and asserts it takes the
    `#humanCommon` path (`opponent-selector.ts:454-457`) — the silent degradation B6d exists
    to avoid and B6a must never emit.
16. **`atPly` fires off-spine.** A P1 candidate played with a `human_common` opponent that
...
17. **Prose support fails closed under B6a.** A record whose `supports` targets
...
18. **Overreach on move grading is mechanically impossible.** A record whose `supports`
    targets `/deviations/0/class` fails `EVIDENCE_OVERREACH`.
19. **A broken pointer is an error, a stale digest is a warning.** Asserted separately, with
    the exit status of each.
20. **No candidate is promotable.** Flipping an emitted candidate's `reviewStatus` to
    `reviewed` without adding a reviewer fails `pack-check` with
    `GRADUATION_REQUIRES_REVIEWERS` (`pack-validation.ts:91-100`).

**Repo hygiene:**

21. `make verify` green; `content/sources/` in `.gitignore`; `content/candidates/README.md`
    stating the directory is candidates-not-content and why it is neither `drafts/` nor
    `packs/`.
22. `docs/` gains a canonical `content-sourcing.md` on implementation, covering B6a and
...
23. `content/drafts/*.json` are **not modified** by this RFC. §4 names what a re-emission
    could fill; doing it is authoring work under `planning/content-era/`, and the friction is
    logged there.
