...
import { RULES_EVIDENCE_FACTS, THEORY_EVIDENCE_FACTS, packAbsentEvidenceRef, packEvidenceRef, rulesEvidenceRef, tempoEvidenceRef, type RulesEvidenceFact, type TheoryEvidenceFact } from "./evidence-ref.js";
import type { EvidencePayload } from "./types.js";

/**
 * The declared-convention resolution behind `run.record.evidence_ref_resolution@1`: an evidence
 * reference token plus the pack and attached payloads determine the displayed text and source
 * label. The factory computes it; no caller may supply the text.
 */
