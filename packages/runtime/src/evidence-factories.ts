...
export const createRulesStructuralReadingNamedStructureV2Evidence = fenPopulation("rules.structural.reading.named_structure@2", (fen) => cachedStructuralReading(fen).structures.map((structure) => Object.freeze({ id: structure.id, name: structure.name, provenanceNote: structure.provenanceNote, squares: Object.freeze([...evaluateNamedStructureWithWitness(fen, structure.id).squares]) })), { dependency: "semantic-convention-provenance", pending: "The structure catalogue is the product-owned convention; its registered descriptor lands with semantic-convention-provenance." });
