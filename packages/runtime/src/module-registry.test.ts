...
describe("module registration — the compiled production registry", () => {
  ...
  it("marks what is executable versus blocked, per pair, and blocks guided hint by name", () => {
    ...
    // evidence-presentation Checkpoint B: every evidence-bearing module except the hint seat now has
    // executable pairs — an operation acquires the source AND an exact adapter presents it.
    expect(new Set(executable.map((pair) => pair.module))).toEqual(new Set(["sight_on_request", "blunder_prevention", "threat_radar", "postcommit_nudge", "structure_nudge", "theory_breadcrumb", "compare_coach", "review_map", "full_inspector"]));
