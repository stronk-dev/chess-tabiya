...
function run(fens: readonly string[], events: DrillRun["events"] = []): DrillRun {
  ...
  return Object.freeze({ schemaVersion: "0.9", id: "adaptive", sessionKind: "position", packId: null, packDigest: null, sessionDigest: "sha256:test", start: { fen: fens[0]!, side: "white" }, feedbackPolicy: "attempt_end", opponentPolicy: { mode: "human_common", targetElo: 1500 }, policyConfig: { seedMode: "fixed", locus: { executedAt: "server", engineIds: [], modelIds: [] } }, nodes, branches: [{ id: "main", forkNodeId: "n0", label: "Main", seed: 1, origin: "played" }], events, activeCursor: { nodeId: nodes.at(-1)!.id, branchId: "main" } } satisfies DrillRun);
}
