...
`make verify` is the required local/CI gate and runs strict type checking, Vitest, and the
current schema/scaffold verification. `make build` separately proves the Svelte production
bundle; it is not part of the RFC-mandated three-part verify target.
