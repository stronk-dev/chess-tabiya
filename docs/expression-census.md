...
With `DECLARATIONS=1`, the report adds a third, opt-in measurement: a declaration census over
four live namespaces — drill-pack schema properties and enum values, `ServerErrorCode`,
`AssistanceConfig` keys and values, and exported runtime reading vocabulary. Each subject keeps
four facts separate:

- **Producers:** production sites that can emit or construct the declared value.
- **Consumers:** production sites that interpret the value as supported input.
- **Refusal sites:** production sites that reject it by name. A refusal is deliberately not a
  consumer; `/retryVariants` is the pinned example.
- **Corpus firings:** authored uses or evaluated transition observations in the current corpus.

The source scan includes production code under `apps/` and `packages/`; it excludes tests,
fixtures, generated output, `node_modules`, content, and disposable `tools/` harnesses. The
declarations themselves are read from the live schema and TypeScript source. A count of zero is
only a measurement: the report never labels a declaration dead, unused, unreachable, or
unsatisfiable, and it emits no authoring issue or suggested remedy.
...
make expression-census DECLARATIONS=1 OUT=/tmp/declaration-census.json
...
The census never writes content and is deliberately absent from `make verify`. Declaration
measurement is report-only: it changes neither `pack-check` nor `shape-check`, and invoking the
command without `DECLARATIONS=1` preserves the original report shape. `validateShapeEntry` does refuse a proven-unsatisfiable trigger or plan signature as `STRUCTURAL_EXPRESSION_UNSATISFIABLE`; this does not remove `probeMatches` from lint responses. A `shape-check PROBE=` invocation prints `PROBE FIRES` or `PROBE DOES NOT FIRE` for every checked trigger. `pack-check` gains no new severity.
