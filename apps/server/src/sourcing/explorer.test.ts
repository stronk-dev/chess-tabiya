...
async function attachableCandidate(): Promise<string> {
  ...
  pack.feedbackClaims = [{ id: "move-frequency", text: "Author placeholder", evidenceTypes: ["corpus_observed"] }];
...
describe("Lichess explorer sourcing", () => {
  ...
  it("attaches the one generated sentence idempotently and re-derives pack identity", async () => {
    const directory = await attachableCandidate();
    const args = { directory, moveSan: "Bf5", target: "/feedbackClaims/0/text", query: { ratings: [1400, 1600, 1800] as const, speeds: ["blitz", "rapid"] as const, since: "2024-01", until: "2026-07" }, client: { stats: fixtureAvailableExplorer } };
    ...
    expect(pack.feedbackClaims[0].text).toContain("31.4% of 128034 games");
    expect(ledger.records).toContainEqual(expect.objectContaining({ kind: "explorer_frequency", templateId: "explorer-move-share/v1", supports: ["/feedbackClaims/0/text"] }));
  ...
  it("leaves all files untouched for forbidden targets, missing moves, and unavailable evidence", async () => {
    ...
    await expect(attachExplorerEvidence({ directory, moveSan: "Qa9", target: "/feedbackClaims/0/text", query, client: { stats: fixtureAvailableExplorer } })).rejects.toMatchObject({ code: "MOVE_NOT_IN_RESPONSE" });
    ...
    await expect(attachExplorerEvidence({ directory, moveSan: "Bf5", target: "/feedbackClaims/0/text", query, client: { stats: fixtureUnavailableExplorer } })).resolves.toBe("abstained");
  ...
  it("fails altered, incomplete, extra, or overreaching template evidence", async () => {
    const directory = await attachableCandidate();
    const query = { ratings: [1400, 1600, 1800] as const, speeds: ["blitz", "rapid"] as const, since: "2024-01", until: "2026-07" };
    await attachExplorerEvidence({ directory, moveSan: "Bf5", target: "/feedbackClaims/0/text", query, client: { stats: fixtureAvailableExplorer } });
    ...
    cleanLedger.records.find((record: any) => record.kind === "explorer_frequency").values.sharePct = 31.5;
    ...
    cleanLedger.records.find((record: any) => record.kind === "explorer_frequency").values.sharePct = 31.4;
    cleanLedger.records.find((record: any) => record.kind === "explorer_frequency").values.extra = 1;
    await writeCanonicalJson(ledgerPath, cleanLedger);
    expect((await checkSourcingDirectory(directory, { strict: true })).issues.map((value) => value.code)).toContain("EVIDENCE_VALUES_INVALID");

    delete cleanLedger.records.find((record: any) => record.kind === "explorer_frequency").values.extra;
    ...
    await writeCanonicalJson(ledgerPath, cleanLedger);
    expect((await checkSourcingDirectory(directory, { strict: true })).issues.map((value) => value.code)).toContain("EVIDENCE_OVERREACH");
  });
});
