...
async function main(): Promise<number> {
  ...
  if (!directory || pipeline !== "explorer") {
    console.error("Usage: make candidate-attach DIR=<candidate> PIPELINE=explorer ARGS='...'");
    return 2;
  }
  try {
    ...
    const result = await attachExplorerEvidence({ directory, ...(args.get("at-spine-node") === undefined || args.get("at-spine-node") === "root" ? {} : { spineNodeId: args.get("at-spine-node")! }), moveSan, target, query, client: { stats: process.env.OFFLINE === "1" ? fixtureAvailableExplorer : (value) => client.stats(value) } });
    console.log(`Explorer attachment ${result}: ${directory}`);
    return 0;
  } catch (error) {
