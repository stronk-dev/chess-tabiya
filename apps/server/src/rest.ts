...
export function createRestHandler(
  ...
  const authoritativeAssistance = async (runId: string, principal: Principal, requested: unknown) => {
    ...
    // The live provider-health snapshot, never constructor presence (provider health §8, criterion 18).
  ...
  return async (request) => {
    try {
      ...
      if (route.action === "modules/query") {
        ...
        // Exactly the /assistance derivation, so the final digest the client holds can match.
        const availability = serverAvailabilityFromProviderHealth(capabilities === undefined ? undefined : (await capabilities.get()).providerHealth);
        let finalized;
        try {
      ...
      if (route.action === "reasoning-review") {
        requireJson(request);
        if (reasoningReviewProvider === undefined) throw new ServerError("VOICE_UNAVAILABLE", "No external reasoning review provider is configured");
