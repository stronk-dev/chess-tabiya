...
if (semanticResearch === undefined || semanticResearch.accepts.length !== 39 || EVIDENCE_MANIFEST.selectionPolicies[0]?.consumer.id !== semanticResearch.id) throw new TypeError("The research semantic-selection consumer is not closed over its 39 exact event bindings");
...
if (counts.join("/") !== "24/141/25/181/39/39/15/1") throw new TypeError(`Semantic evidence closure drift: ${counts.join("/")}`);

console.log(`evidence-manifest-check: ${EVIDENCE_MANIFEST.digest} · 24/141/25/181 core · 39/39/15/1 semantic`);
