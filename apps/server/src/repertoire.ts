...
export class RepertoireService{
  ...
  gaps(id:string,principal:Principal){const record=this.#owned(id,principal),scan=this.#required("repertoireScan")(id);if(scan===undefined)return Object.freeze({status:this.#jobs.has(id)?"pending":"never_scanned",repertoire:this.summary(record),scan:null});const moves=this.#required("repertoireMoves")(id);const enrich=(raw:unknown)=>Array.isArray(raw)?raw.map((item)=>{const gap=item as GapRow;const link=this.#required("repertoireGapRun")(id,gap.key),answer=moves.find((move)=>move.positionKey===gap.key&&move.rank===0),firstMoves=link===undefined?Object.freeze([]):this.#required("repertoireGapFirstMoves")(link.runId);return Object.freeze({...gap,state:answer!==undefined?"answered":link!==undefined&&this.#required("repertoireGapAttemptCount")(link.runId)>0?"addressed":"open",runId:link?.runId??null,firstMoves,answer:answer===undefined?null:Object.freeze({moveUci:answer.moveUci,moveSan:answer.moveSan})});}):[];return Object.freeze({status:"ready",stale:scan.repertoireDigest!==record.digest,repertoire:this.summary(record),scan:Object.freeze({...scan,gaps:Object.freeze(enrich(scan.gaps)),alternateGaps:Object.freeze(enrich(scan.alternateGaps)),guard:CORPUS_GUARD,partiality:scan.truncated?`Scan stopped after ${scan.queriesUsed} corpus queries; ${scan.unreachedKeys} positions unexplored`:scan.sourceFailures>0?`${scan.sourceFailures} corpus positions were unavailable`:null})});}
  ...
  recommendations(principal:Principal){const rows=[] as Array<Record<string,unknown>>;for(const record of this.#required("repertoires")(principal.learnerId)){const scan=this.#required("repertoireScan")(record.id);if(scan===undefined||!Array.isArray(scan.gaps))continue;for(const gap of scan.gaps as GapRow[]){if(this.#required("repertoireGapRun")(record.id,gap.key)!==undefined)continue;rows.push(Object.freeze({kind:"repertoire_gap",repertoireId:record.id,repertoireName:record.name,gapKey:gap.key,replySan:gap.replySan,line:gap.line,mass:gap.mass,gamesUntilSeen:gap.gamesUntilSeen,sentence:`Your repertoire ${record.name} has no answer to ${gap.replySan} after ${gap.line.join(" ")}; this population reached it about once every ${gap.gamesUntilSeen} games. ${CORPUS_GUARD}`}));}}const eligible=rows.sort((left,right)=>Number(right.mass)-Number(left.mass)||String(left.gapKey).localeCompare(String(right.gapKey)));return Object.freeze({recommendations:Object.freeze(eligible.slice(0,10)),total:eligible.length});}
