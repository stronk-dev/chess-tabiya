...
export class RepertoireService{
  ...
  recommendations(principal:Principal){const rows=[] as Array<Record<string,unknown>>;for(const record of this.#required("repertoires")(principal.learnerId)){const scan=this.#required("repertoireScan")(record.id);if(scan===undefined||!Array.isArray(scan.gaps))continue;for(const gap of scan.gaps as GapRow[]){if(this.#required("repertoireGapRun")(record.id,gap.key)!==undefined)continue;rows.push(Object.freeze({kind:"repertoire_gap",repertoireId:record.id,repertoireName:record.name,gapKey:gap.key,replySan:gap.replySan,line:gap.line,mass:gap.mass,gamesUntilSeen:gap.gamesUntilSeen,sentence:`Your repertoire ${record.name} has no answer to ${gap.replySan} after ${gap.line.join(" ")}; this population reached it about once every ${gap.gamesUntilSeen} games. ${CORPUS_GUARD}`}));}}return Object.freeze(rows.sort((left,right)=>Number(right.mass)-Number(left.mass)||String(left.gapKey).localeCompare(String(right.gapKey))).slice(0,10));}
