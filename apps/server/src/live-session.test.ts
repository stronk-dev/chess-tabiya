...
describe("live session platform",()=>{
  ...
  it("migrates to the live schema and enforces board control, proposals, and namespaced advisory votes",async()=>{
    const {storage,queue,handler}=setup();expect(STORAGE_VERSION).toBe(25);
    ...
    await request(handler,"POST",`/sessions/${session.id}/votes`,{cookie:bob.cookie,body:{op:"cast",windowId:window.id,choiceUci:"e2e4"}});
    for(const voterKey of ["viewer-1","viewer-2","viewer-3"]){expect((await request(handler,"POST",`/sessions/${session.id}/votes`,{cookie:chat.cookie,body:{op:"cast",windowId:window.id,choiceUci:"d2d4",voterKey}})).status).toBe(200);}
    const relay=await request(handler,"POST",`/sessions/${session.id}/votes`,{cookie:chat.cookie,body:{op:"cast",windowId:window.id,choiceUci:"e2e4",voterKey:"viewer-1"}});expect(relay.status).toBe(200);const tally=await relay.json() as any;expect(tally.total).toBe(5);expect(tally.relayed).toBe(3);expect(tally.relayed).toBeLessThanOrEqual(tally.total);expect(tally.tally.map((item:any)=>item.count)).toEqual([3,2]);expect(JSON.stringify(storage.read("live-run")!.run.events)).toBe(before);
    const detail=await (await request(handler,"GET",`/sessions/${session.id}`,{cookie:alice.cookie})).json() as any;expect(detail.activeFen).toBe(FEN);expect(detail.voteAdapter).toEqual({learnerId:chat.learner.id,handle:"chatbridge"});expect(detail.vote).toMatchObject({total:5,relayed:3});
  ...
  it("alternates a native match, gates live rehearsal, and seats a friend through one-use public tokens",async()=>{
    ...
    const summaries=await (await request(handler,"GET","/sessions",{cookie:coach.cookie})).json() as any;expect(summaries.sessions[0].board).toMatchObject({objectiveState:"active",sideToMove:"black",plyCount:3,pausedAt:null,lastMoveAt:expect.any(String),players:{white:{handle:"white"},black:{handle:"black"}}});
