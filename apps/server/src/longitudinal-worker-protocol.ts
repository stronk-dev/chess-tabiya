...
export type LongitudinalThreadMessage =
  ...
  | {
    ...
    readonly conflicts: number; readonly renewals: number;
  }
...
export function parseLongitudinalThreadMessage(value: unknown): LongitudinalThreadMessage | undefined {
  ...
  switch (message.type) {
    ...
    case "progress":
      return keys === "claimed,completed,conflicts,failed,renewals,type" && ["claimed", "completed", "conflicts", "failed", "renewals"].every(count)
        ? message as LongitudinalThreadMessage : undefined;
    case "drained":
