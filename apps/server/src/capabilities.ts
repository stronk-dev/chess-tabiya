...
export const CAPABILITY_DISPOSITIONS: readonly CapabilityDisposition[] = Object.freeze([
  ...
  { instrument: "Stockfish", capability: "all-legal MultiPV for fixed-depth legal-root measurement", disposition: "reached", reason: "stockfish.legal_root_table@1 sets MultiPV to the exact legal-move count at one requested depth, records every root row in its own side-to-move frame, and resets MultiPV 1 in finally (rfc/provider-exchange-and-execution.md §5); operator-only until a named consumer compiles", surface: "provider exchange", advertisedOptions: ["MultiPV"] },
  { instrument: "Stockfish", capability: "MultiPV > 1 outside enumerate and the all-legal legal-root measurement", disposition: "refused", reason: "No attested need for any other multi-line search; the legal-root permission does not widen to top-k or ranked lines", advertisedOptions: ["MultiPV"] },
  ...
  { instrument: "Stockfish", capability: "UCI_Chess960", disposition: "unmeasured", reason: "Variants are approved by surface: drills remain standard, while Just Play, import/analysis, and campaign support await the accepted variants implementation", experiment: "rfc/variants.md acceptance and Chess960 application-boundary fixtures", advertisedOptions: ["UCI_Chess960"] },
  ...
  { instrument: "Glicko-2", capability: "rating from authored, engine- or tablebase-adjudicated outcomes", disposition: "refused", reason: "A pack's declared success is not a game result, and a tablebase result is a fact about optimal play rather than about the game" },
  { instrument: "Glicko-2", capability: "rating as an input to what is said about a move", disposition: "refused", reason: "A rating may select what a learner is shown and is never an argument to a rendering" },
  { instrument: "Glicko-2", capability: "cross-learner comparison outside a joined cohort", disposition: "refused", reason: "A standing spans one classroom the learner published themselves into; there is no global table" },
  ...
  // rfc/famous-games.md §1 (owner ruling D1060, FULL LIFT): four rows replace the one bundled row.
  { instrument: "Explorer", capability: "topGames / recentGames", disposition: "refused", reason: "Product scope only: the corpus panel renders population results, not games, and has no per-game consumer; a per-game corpus surface is a revisitable product judgement (rfc/famous-games.md D5)" },
  { instrument: "Explorer", capability: "masters database position and per-move aggregates", disposition: "reached", reason: "Same terms as the Lichess explorer: no rights asserted, no attribution required; aggregate statistics are facts. Fetched only as topGames=0 aggregates through the explorer client's serialized retry-and-abstain path", surface: "authoring sourcing lane" },
  { instrument: "Explorer", capability: "masters per-game PGN (explorer.lichess.org/masters/pgn/{id})", disposition: "reached", reason: "A bare game score is a record of fact. Hand-selected games only, one id per sourcing invocation; a systematic walk of the masters index is refused (EU database right, Directive 96/9/EC Art. 7(5))", surface: "authoring sourcing lane" },
  { instrument: "Imported game", capability: "third-party annotations, NAGs and move verdicts", disposition: "refused", reason: "Commentary is copyrightable expression and another product's verdict on a move (law 8). Stripped at the record boundary on the Lichess game, study and masters fetches (D410); the manual-paste path still retains raw bytes and is D959's open row" },
