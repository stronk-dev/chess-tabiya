...
describe("SQLite run-storage migrations and summaries", () => {
  ...
  it("keeps ordinary v0.5 runs readable while quarantining pre-producer outcomes", () => {
    ...
    expect(upgraded.read(ordinary.id)?.run.schemaVersion).toBe("0.16");
  ...
  it("upgrades storage 15 run snapshots to 0.11 once and skips on reopen", () => {
    ...
    expect(log).toEqual([{ version: 16, name: "immediate guard run schema" }, { version: 17, name: "stated reasoning run schema" }, { version: 18, name: "perfect tablebase run schema" }, { version: 19, name: "practical resistance run schema" }, { version: 20, name: "engine request record run schema" }, { version: 21, name: "engine leverage run schema" }]);
    expect(upgraded.read("guard-migration")?.run.schemaVersion).toBe("0.16");
    upgraded.close();
  ...
  it("stamps v0.13 snapshots to v0.14 without rewriting selection evidence", () => {
    ...
    const log: StorageMigrationLog[] = [];
    const upgraded = new SQLiteRunStorage(filename, { onMigration: (entry) => log.push(entry) });
    expect(log).toEqual([{ version: 19, name: "practical resistance run schema" }, { version: 20, name: "engine request record run schema" }, { version: 21, name: "engine leverage run schema" }]);
    const after = upgraded.read(selected.id)!.run;
    expect(after.schemaVersion).toBe("0.16");
  ...
  it("stamps v0.14 snapshots to v0.15 without inventing off-window candidates", () => {
    ...
    const log: StorageMigrationLog[] = [];
    const upgraded = new SQLiteRunStorage(filename, { onMigration: (entry) => log.push(entry) });
    expect(log).toEqual([{ version: 20, name: "engine request record run schema" }, { version: 21, name: "engine leverage run schema" }]);
    const after = upgraded.read(selected.id)!.run;
    expect(after.schemaVersion).toBe("0.16");
    expect(after.events).toEqual(selected.events);
  ...
  it("stamps v0.15 snapshots to v0.16 without inventing engine leverage evidence", () => {
    ...
    const log: StorageMigrationLog[] = [];
    const upgraded = new SQLiteRunStorage(filename, { onMigration: (entry) => log.push(entry) });
    expect(log).toEqual([{ version: 21, name: "engine leverage run schema" }]);
    const after = upgraded.read(selected.id)!.run;
    expect(after.schemaVersion).toBe("0.16");
    expect(after.events).toEqual(selected.events);
