...
describe("SQLite run-storage migrations and summaries", () => {
  ...
  it("keeps ordinary v0.5 runs readable while quarantining pre-producer outcomes", () => {
    ...
    expect(upgraded.read(ordinary.id)?.run.schemaVersion).toBe("0.13");
  ...
  it("upgrades storage 15 run snapshots to 0.11 once and skips on reopen", () => {
    ...
    expect(log).toEqual([{ version: 16, name: "immediate guard run schema" }, { version: 17, name: "stated reasoning run schema" }, { version: 18, name: "perfect tablebase run schema" }]);
    expect(upgraded.read("guard-migration")?.run.schemaVersion).toBe("0.13");
    upgraded.close();
