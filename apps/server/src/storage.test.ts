...
describe("SQLite run-storage migrations and summaries", () => {
  ...
  it("upgrades and backfills a legacy fixture once, then skips on reopen", () => {
    ...
    expect(firstLog).toEqual([
      ...
      { version: 26, name: "longitudinal observation ledger, structure stats, and projection jobs" }, { version: 27, name: "durable evidence job batches, jobs, result sequences and application transitions" }, { version: 28, name: "registered global concept identities and the legacy concept quarantine" }, { version: 29, name: "bot profile run schema" },
    ]);
  ...
  it("keeps ordinary v0.5 runs readable while quarantining pre-producer outcomes", () => {
    ...
    expect(upgraded.read(ordinary.id)?.run.schemaVersion).toBe("0.18");
  ...
  it("upgrades storage 15 run snapshots to 0.11 once and skips on reopen", () => {
    ...
    expect(upgraded.read("guard-migration")?.run.schemaVersion).toBe("0.18");
    upgraded.close();
