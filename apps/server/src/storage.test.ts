...
describe("SQLite run-storage migrations and summaries", () => {
  ...
  it("adds the six rating and cohort tables without rating historical learners or runs", () => {
    ...
    expect(log).toEqual([{ version: 25, name: "learner ratings, rated games, periods, standings, and marks" }, { version: 26, name: "longitudinal observation ledger, structure stats, and projection jobs" }]);
    upgraded.close();
  ...
  it("upgrades and backfills a legacy fixture once, then skips on reopen", () => {
    ...
    expect(firstLog).toEqual([
      ...
      { version: 26, name: "longitudinal observation ledger, structure stats, and projection jobs" },
    ]);
  ...
  it("stamps v0.16 snapshots to v0.17 without inferring an ordering basis", () => {
    ...
    const log: StorageMigrationLog[] = [];
    const upgraded = new SQLiteRunStorage(filename, { onMigration: (entry) => log.push(entry) });
    expect(log).toEqual([{ version: 23, name: "opponent ordering basis run schema" }, { version: 24, name: "classrooms, assignments, submissions, and expiring run grants" }, { version: 25, name: "learner ratings, rated games, periods, standings, and marks" }, { version: 26, name: "longitudinal observation ledger, structure stats, and projection jobs" }]);
    const after = upgraded.read(selected.id)!.run;
    expect(after.schemaVersion).toBe("0.17");
    expect(after.events).toEqual(selected.events);
    expect(after.events.some((event) => event.type === "opponent.move_selected" && event.data.selection.orderingBasis !== undefined)).toBe(false);
