...
>
  ...
  {:else if route.name === "library"}
    <main class="shell-view" aria-labelledby="library-title">
      ...
      <section><h2>Rehearsal packs</h2><ul>{#each packs as pack}<li>{pack.title} <small>{labelOrFallback("pack_review_status", pack.reviewStatus, "review status not recorded")}</small></li>{:else}<li>No packs available.</li>{/each}</ul></section>
