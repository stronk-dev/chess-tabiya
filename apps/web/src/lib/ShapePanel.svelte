...
<style>
  ...
  header{display:flex;justify-content:space-between;gap:1rem;align-items:start}h2,h3,h4,p{margin:.35rem 0}h2{font:500 2rem/1 var(--display-font)}h3{margin-top:1.2rem}.eyebrow,footer{color:var(--muted);font-size:.78rem}.eyebrow{text-transform:uppercase;letter-spacing:.1em}.frame{padding:.65rem;border-left:3px solid var(--accent);background:var(--paper-soft)}article{margin:.7rem 0;padding:.7rem;border:1px solid var(--line);border-radius:.7rem}.lists{display:grid;grid-template-columns:1fr 1fr;gap:1rem}footer{margin-top:1rem;padding-top:.6rem;border-top:1px solid var(--line)}button{padding:.5rem .65rem;border:1px solid var(--line);border-radius:.5rem;background:transparent;color:inherit}
</style>
