...
describe("client surface floor", () => {
  ...
  it("keeps every non-host assistance permission pointwise at or below the host ceiling", () => {
    const rank = { locked_off: 0, legal: 1, free: 1, sight: 1, evidence: 2 } as const;
    for (const deliveryOpen of [false, true]) {
      const host = permittedAssistance({ workflowContext: "position", deliveryOpen, role: "host", seatedInContest: false, reviewing: false });
      const solo = permittedAssistance({ workflowContext: "position", deliveryOpen, role: "solo", seatedInContest: false, reviewing: false });
      expect(host).toEqual(solo);
      for (const role of ["participant", "spectator"] as const) {
        const candidate = permittedAssistance({ workflowContext: "position", deliveryOpen, role, seatedInContest: false, reviewing: false });
