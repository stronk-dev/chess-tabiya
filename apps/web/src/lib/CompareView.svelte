...
<section class="compare" aria-labelledby="compare-title">
  ...
  <section class="narrative" aria-label="Grounded comparison">
    <div class="narrative-heading"><div><p class="eyebrow">Grounded comparison</p><h3>What the recorded attempts show</h3></div><button type="button" aria-expanded={narrativeOpen} onclick={() => narrativeOpen = !narrativeOpen}>{narrativeOpen ? "Hide" : "Show"}</button></div>
    {#if narrativeOpen}{#each narrative.groups as group}<div>{#each group.sentences as sentence}<p>{sentence}</p>{/each}</div>{/each}{#if onVoice}<button type="button" onclick={() => void onVoice().then((text) => personaText = text)}>Revoice grounded comparison</button>{/if}{#if personaText}<p>{personaText}</p>{/if}{/if}
  </section>
      ...
      <article>
        ...
        {#if consequence}
          <details><summary>Opponent and authored-line context</summary>
            ...
            {#each consequence.theory ?? [] as entry}<p>{theorySentence(entry)}</p>{:else}<p>No authored line is attached to this comparison.</p>{/each}
          </details>
          <details data-evidence-consumer="inspector.position_structure"><summary>Position structure facts</summary>
