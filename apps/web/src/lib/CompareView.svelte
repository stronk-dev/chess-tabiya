...
<section class="compare" aria-labelledby="compare-title">
  ...
  {#if comparison.machineFeedback === "withheld"}
    <p class="feedback-withheld" data-abstention="feedback_withheld" role="status">
      Engine comparison is withheld until this attempt reaches its disclosure boundary. Recorded paths, positions, and objective changes remain available.
    </p>
  {/if}
        ...
        {#if group.node}
          ...
          {#if zoom === "mid" || zoom === "near"}
            <dl>
              ...
              <div><dt>Played from the rehearsal start</dt><dd>{rehearsalTurnCount(group.node.ply)}</dd></div>
  ...
  <section class="results" aria-label="Per-branch consequences">
    {#each comparison.columns as column}
      ...
      <article>
        <h3>{column.label}</h3>
        <p>{consequence?.decision ? `Decision: ${consequence.decision.moveSan}. ${rehearsalTurnCount(consequence.decision.plyOffset)} from the shared fork.` : "No moves on this branch yet."}</p>
