...
export function renderDeployment({ serverImage, maiaImage, root = ROOT }) {
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (out === undefined || serverImage === undefined || maiaImage === undefined) {
    console.error("usage: render-deployment.mjs --out <dir> --server-image <ref> --maia-image <ref>");
    process.exit(2);
  }
  ...
  for (const [name, text] of Object.entries(renderDeployment({ serverImage, maiaImage }))) writeFileSync(join(out, name), text);
