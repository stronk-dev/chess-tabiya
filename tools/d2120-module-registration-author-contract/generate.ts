...
const MODULE_VIEW_RESOLVER = Object.freeze({
...
const SOURCE_CONTRACTS = Object.freeze([
  ...
  { id: "review_evidence_packet@1", authority: "review-evidence-compiler", input: "ReviewEvidenceInput", result: "ReviewEvidencePacket", operation: { owner: "rfc/review-evidence-compiler.md", callable: "compileReviewEvidence(input)" }, extract: "projection-keyed frozen-prefix occurrences", views: ["recorded_edge", "recorded_branch_pair", "frozen_run_prefix"], viewAuthority: MODULE_VIEW_RESOLVER, forbiddenViews: ["current_root", "hypothetical_candidate"], assertion: { callable: "assertReviewEvidencePacket(value)", appliesTo: "ReviewEvidencePacket" }, itemAssertion: "assertDeclaredEvidence(value)", abstain: "ReviewFamilyState", seal: "private aggregate authority issued only by createReviewEvidencePacket", subjectKinds: ["edge", "branch_pair", "run_prefix"], timings: ["postcommit", "review", "analysis"], blocker: "review-evidence-compiler production implementation must land before exact module operation resolution", status: "awaiting_upstream_sealed_operation" },
...
const SOURCE_INPUTS = Object.freeze([
  ...
  ["run.record.position", "position", "recorded_semantic_path@1"],
...
const sourceFor = (producer: string, stage: string, subjectKind: string): string => {
  ...
  if (subjectKind === "run_prefix") return "recorded_semantic_path@1";
...
const EXACT_OCCURRENCE_CONTRACTS = Object.freeze({
  "derived.tactic.deflection_observed": { kind: "ordered_window", alternatives: [
    { discriminator: "bait_capture", horizon: 3, outputEdgeOffset: 3, operands: [
    ...
    { discriminator: "check_induced", horizon: 3, outputEdgeOffset: 3, operands: [
      { projection: "run.record.move", cardinality: 3, edgeOffsets: [1, 2, 3], roles: ["checking_bait_move", "defender_reply", "target_capture"] },
      { projection: "rules.tactic.reading.defender_duty_set", cardinality: 1, positionOffsets: [0], roles: ["duty_before"] },
      { projection: "rules.tactic.event.check", cardinality: 1, edgeOffsets: [1], roles: ["checking_bait_move"] },
      { projection: "rules.transition.event.capture", cardinality: 1, edgeOffsets: [3], roles: ["target_capture"] },
      { projection: "rules.exchange.predicate.legal_exchange", cardinality: 1, edgeOffsets: [3], roles: ["positive_target_capture"] },
    ] },
  ...
  "derived.compare.eval_delta": { kind: "ordered_endpoints", alternatives: [{ operands: [
    { projection: "live.stockfish.eval", cardinality: 2, endpointRoles: ["before", "after"] },
  ] }], equality: ["same_recorded_branch", "consecutive_trail_order", "same_engine_id", "same_search_limit", "same_score_domain"], outputOperands: ["delta", "plyOffset"] },
...
const requirementRows = compiledIds.map((id) => {
  ...
  const source = sourceContractById.get(sourceFor(producer.id, stage, subjectKind));
...
for (const row of requirementRows) {
  ...
  const inputBindings = inputs.map((input) => {
    ...
    const occurrenceOperands = exactOccurrence?.alternatives.map((alternative) => Object.freeze({
      ...(alternative.discriminator === undefined ? {} : { discriminator: alternative.discriminator }),
      operands: Object.freeze(alternative.operands.filter((operand) => operand.projection === input.id)),
    })).filter((alternative) => alternative.operands.length > 0);
    return { projection: input, sourceSubjectKind,
      relation: exactOccurrence === undefined ? inputRelation(sourceSubjectKind, row.subjectKind) : "ordered_window_operand",
      ...(occurrenceOperands === undefined ? {} : { occurrenceOperands }) };
  });
...
const CANDIDATE_MOMENTS = new Set(["precommit", "at_commit"]);
const occurrenceSourceForMoment = (requirement: (typeof requirementRows)[number], timing: string) => {
  const sourceId = requirement.acquisition === "candidate_population@1" && !CANDIDATE_MOMENTS.has(timing)
    ? "recorded_semantic_path@1"
    : requirement.acquisition;
  const source = sourceContractById.get(sourceId);
  if (source === undefined || !source.timings.includes(timing as never) || !source.subjectKinds.includes(requirement.subjectKind as never)) return undefined;
  return Object.freeze({
    timing,
    source: source.id,
    view: occurrenceViewFor(source.id, requirement.subjectKind),
...
const bindingRows = pairs.map(({ module, projection: projectionId }) => {
  ...
  const occurrenceByMoment = policy.timings.flatMap((timing) => {
    const occurrence = occurrenceSourceForMoment(requirement, timing);
    return occurrence === undefined ? [] : [occurrence];
  });
  const sourceTimingCeiling = occurrenceByMoment.map((value) => value.timing);
  if (sessions.length === 0 || forms.length === 0 || occurrenceByMoment.length === 0) throw new TypeError(`empty policy intersection: ${module}/${projectionId}`);
  ...
  return { producer: projection.producer, projection: ref(projection.id, projection.version), consumer: ref(`module.${module}`),
    occurrenceRequirement: {
      byMoment: occurrenceByMoment,
      exactProjectionOperation: null,
      resolutionOwner: { ...MODULE_VIEW_RESOLVER, projection: ref(projection.id, projection.version) },
...
const exactOperationResolution = Object.freeze({
  ...MODULE_VIEW_RESOLVER,
  requiredProjectionKeys: requirementRows.map((row) => `${row.projection.id}@${row.projection.version}`).sort(),
  requiredPairKeys: bindingRows.map((row) => `${row.consumer.id}@${row.consumer.version}\u0000${row.projection.id}@${row.projection.version}`).sort(),
