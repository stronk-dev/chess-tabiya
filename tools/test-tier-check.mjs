...
import { CONTENT_CONTRACT_TESTS } from "./test-tiers.mjs";
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  if (errors.length > 0) {
    ...
    console.log(`test-tier-check: ${CONTENT_CONTRACT_TESTS.length} real-content test files are isolated from software contracts`);
  }
}
