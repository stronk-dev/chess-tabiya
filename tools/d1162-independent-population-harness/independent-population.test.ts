...
function markdown(result: ReturnType<typeof run>): string {
  return `# D1162 independent-population result\n\nFormal preregistered verdict: **${result.verdict}**. ${String(result.population.positions)} decisions from ${String(result.population.games)} games; ${String(result.population.legalCandidates)} legal candidates; ${(result.population.legalSetCoverage * 100).toFixed(0)}% legal-set identity.\n\n| contrast | mean game-level played-move probability delta | game-bootstrap 95% CI |\n|---|---:|---:|\n| evidence-only − uniform | ${result.primary.evidenceVsUniform.mean.toFixed(6)} | [${result.primary.evidenceVsUniform.ci95.join(", ")}] |\n| evidence + engine − engine-only | ${result.primary.combinedVsEngine.mean.toFixed(6)} | [${result.primary.combinedVsEngine.ci95.join(", ")}] |\n\nThe formal pass does **not** clear the fitted head: combined cross entropy is ${result.summary.combined.crossEntropy.toFixed(6)} against engine-only ${result.summary.engine.crossEntropy.toFixed(6)}, top-choice agreement is ${(100 * result.summary.combined.topAgreement).toFixed(1)}% against ${(100 * result.summary.engine.topAgreement).toFixed(1)}%, expected loss is ${result.summary.combined.expectedLossCp.toFixed(1)} cp against ${result.summary.engine.expectedLossCp.toFixed(1)} cp, and >250-cp mass is ${(100 * result.summary.combined.severe250).toFixed(1)}% against ${(100 * result.summary.engine.severe250).toFixed(1)}%. The representation signal replicated; the diagonal fitting rule and its non-proper primary gate are returned for repair before multi-ply work. This is not a human-like, personality, Elo, skill or production claim.\n`;
}
