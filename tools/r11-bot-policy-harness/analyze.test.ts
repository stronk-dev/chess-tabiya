...
const RESULT = new URL("../../planning/platform-alignment/bot-policy/results.json", import.meta.url);
const REPORT = new URL("../../planning/platform-alignment/bot-policy/results.md", import.meta.url);
...
function loadCells(directory: string): { readonly cells: readonly Cell[]; readonly digests: Record<string, string>; readonly counts: Record<string, number> } {
  const paths = {
    maia: `${directory}/armA-history.jsonl`, sf: `${directory}/sf-d12.jsonl`,
...
describe("R11 bot-policy transforms", () => {
  ...
  it.skipIf(INPUT_DIR === undefined)("measures every predeclared arm over the fixed population", () => {
    ...
    expect(loaded.counts).toEqual({ maiaRows: 837, sfRows: 279, probePositions: 279, cells: 837 });
    ...
    const result = {
      measuredAt: new Date().toISOString(), parameters: PARAMETERS, inputs: loaded.digests,
