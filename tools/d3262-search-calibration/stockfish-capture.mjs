...
const frontierMode = horizon4Mode || historySupplementMode || coherentDeeperSupplementMode;
...
if (Number(args.includes("--child")) + Number(coherentRootMode) + Number(coherentChildMode) + Number(coherentAllRootMode) + Number(coherentAllChildMode) + Number(coherentNewChildMode) + Number(coherentNewAllChildMode) + Number(horizon4Mode) + Number(historySupplementMode) + Number(coherentDeeperSupplementMode) > 1) throw new Error("Select one Stockfish capture population");
...
const population = frontierMode
  ? (coherentDeeperSupplementMode ? frontier.engineJobs : frontier.jobs).map((job) => ({ jobId: job.id, fen: job.fen }))
  : childMode
