...
export function readPackPhase(pack: DrillPackDefinition): PackPhaseReading {
  ...
  const visit = (
    ...
    parent: DetectedPhase,
    grandparent: DetectedPhase | undefined,
  ): void => {
    for (const node of nodes) {
      ...
      if (grandparent !== undefined && current.detected === grandparent && current.detected !== parent) {
        twoEdgeReversals += 1;
      }
...
function countDetected(samples: readonly PhaseSample[]): Readonly<Record<DetectedPhase, number>> {
  return Object.freeze(Object.fromEntries(
    (["opening", "middlegame", "endgame", "unclear"] as const).map((phase) => [
      phase,
      samples.filter((row) => row.detected === phase).length,
    ]),
...
export function phaseCensus(packs: readonly DrillPackDefinition[]) {
  ...
  return Object.freeze({
    ...
    convention: Object.freeze({
      endgameMaterialMax: 13,
      developedMaterialMin: 18,
      openingUndevelopedMin: 5,
      middlegameUndevelopedMax: 2,
    }),
