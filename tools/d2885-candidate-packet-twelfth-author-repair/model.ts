// DISPOSABLE composed author model for D2885-D2891. Not production code.
...
class CandidateCollectorFailure extends Error {
  constructor(readonly moveUci: string, readonly projection: Projection, readonly reason: "threw" | "invalid_result") { super("CANDIDATE_COLLECTOR_FAILED"); }
}
...
function invokeCollector(collectorId: CollectorId, context: CollectorContext): Readonly<{ kind: "available"; values: readonly EvidenceValue[] } | { kind: "unavailable"; reason: "invalid_turn_clone" }> {
  try {
    ...
    if (error instanceof CandidateCollectorFailure) throw error;
    throw new CandidateCollectorFailure(context.moveUci, CANDIDATE_COLLECTOR_PROJECTION_KEYS[collectorId][0] as Projection, "threw");
  }
}
...
function normalizeCollector(collectorId: CollectorId, context: CollectorContext, retain: boolean) {
  ...
  try {
    ...
    throw new CandidateCollectorFailure(context.moveUci, expected[0]!, "invalid_result");
  }
  const seen = new Set(values.map(projectionKey));
  const undeclared = [...seen].find((value) => !expected.includes(value as Projection));
  if (undeclared !== undefined) throw new CandidateCollectorFailure(context.moveUci, expected[0]!, "invalid_result");
...
function executeCandidateSync(beforeFen: string, move: ExactLegalMove, plan: ReturnType<typeof planCandidateCollectors>): CandidatePacketInput {
  ...
  for (const item of plan) {
    const normalized = normalizeCollector(item.collectorId, seal({ beforeFen, moveUci: move.uci, afterFen, memo: seal({ ...memo }) }), item.retain);
...
export function assertCandidatePopulationReceipt(value: unknown): asserts value is CandidatePopulationReceipt {
  ...
  if (receipt.packet.legalMoves.length !== receipt.candidateInputs.length || receipt.packet.candidates.length !== receipt.candidateInputs.length) throw new TypeError("CANDIDATE_LEGAL_SET_CROSSED");
...
function service(limits: CandidatePopulationServiceLimits, compile: CompileHook): CandidatePopulationService {
  ...
  const finish = (job: Job, result: CandidatePopulationResult) => {
    ...
    job.settled = true;
    if (job.queueTimer !== undefined) { clearTimeout(job.queueTimer); job.queueTimer = undefined; }
