// DISPOSABLE composed author contract for D2846-D2851. Not production code.
...
export type ExchangeDelivery<T> = Readonly<ExchangeRequest & { payload: T; responseDigest: string }>;
...
export class ProviderRegistry {
  ...
  assertGenerationSet(value: ProviderGenerationSet, group: BackoffGroupId): void {
    ...
    const current = this.snapshot(this.#lastNow);
    const expected = this.generationSet(current, group);
    if (expected.digest !== value.digest) fail("GENERATION_SET_STALE_OR_CROSSED");
  }
...
export class BackoffCoordinator {
  ...
  settle(now: number, generations: ProviderGenerationSet, token: string, settlement: BackoffSettlement): void {
    this.#assertClaim(now, generations, token); this.#claim = null;
    if (settlement.kind === "success") { this.#transientFailures = 0; this.#blockedUntil = 0; return; }
    if (settlement.kind === "rate_limited") { if (settlement.retryAfterMs !== null && (!Number.isSafeInteger(settlement.retryAfterMs) || settlement.retryAfterMs < 0)) fail("RETRY_AFTER_INVALID"); this.#blockedUntil = now + Math.max(60_000, settlement.retryAfterMs ?? 0); return; }
    if (settlement.kind === "permanent_failure") { this.#blockedUntil = Number.MAX_SAFE_INTEGER; return; }
...
function deepFreeze<T>(value: T): T { if (typeof value !== "object" || value === null || Object.isFrozen(value)) return value; for (const child of Object.values(value as object)) deepFreeze(child); return Object.freeze(value); }
