...
**Amendment — O1/O2/O4 RULED 2026-08-20 (owner; written here 2026-08-21 by
claude on those rulings).** The raw list above is the **advanced analysis
inspector inventory** — every producer a curious learner, author or operator may
open — **not the ordinary learner control surface**. Ordinary workflows consume
**named evidence modules and opinionated presets**; raw source switches live in
Advanced/Custom surfaces and the inspector (`05-in-run-experience.md`
§Presentation). The evidence authority is O1's **versioned compiled
producer→evidence→consumer manifest**: producer and module declarations are
primary, joins/landed state are derived, and the manifest declares which
producer/projection may serve which module under **consumer, timing, role,
session, form and answer-distance constraints**; unexplained orphans fail the
compile unless explicitly disposed `inspector_only`, `author_only`,
`operator_only`, `experimental` or `retired`. **LLM wording is never a layer of
chess truth and never selects or grades evidence** (O1; R5's measured boundary
lives in `05` §3b-i). Exact learner-facing detector families, preset names,
budgets and defaults are deliberately not chosen here — they remain with
R3/R7 and their decision nodes.
...
|---|---|---|
| B1 — shell and entry | stable shell routes Play/Learn/Review/Live/Create/Library/Settings; resume works | shipped — shell, routes, resume; `phase` projected (D6 closed by `defect-sweep`). ~~Residual: `/settings` remains display-only~~ **Corrected 2026-08-21 (owner-approved D650): stale in the OPPOSITE direction** — `/settings` renders **54 live controls** (measured hands-on 2026-08-16, [[D484]]; [[D311]] recorded the correction and this row was never updated). The true residual is quality, not capability: unstyled natives, no presets, checkbox-above-label — owned by the R3/R7 presentation work and `assistance-controls` |
...
| B3 — review | manual multi-branch selection, pair/multi compare, replay, deep mode, share/export, **and branch groups played in parallel from a seeded candidate set with resistance held constant** | shipped in full — N-way compare, simulate, prediction rendering, deep analysis, export, **and branch groups (2026-08-14)**. ~~Residual: narrative mode + difference strips (forward-trace orphan, ledgered)~~ **Corrected 2026-08-21 (owner-approved D650): both shipped** — `CompareView.svelte:130` renders *"Per-branch difference strips"* and the narrative section renders behind `narrativeOpen` (`CompareView.svelte:30`, asserted in `screens.test.ts`) `[V]`. The live residual moved tiers: the strip's *content* fails the usefulness axis ([[D78]]: 99.9% firing, lift 1.004×; the CR1 admission filter and [[D543]]'s lift ordering own the fix) — the surface exists, the selection does not |
| B4 — evidence | authored, Stockfish, Maia, corpus/historical, Syzygy, structural/temporal, and LLM-rendered layers work with timing controls | authored ✓, Stockfish ✓, Maia ✓, **corpus/recency ✓ (`runtime-corpus-evidence`)**, structural ✓ (B9), voice seam ✓. True residual **(narrowed 2026-08-21, owner-approved D650)**: ~~Syzygy runtime rendering~~ — tablebase evidence **renders** (`evidence-sentences.ts:163-188` emits *"Exact tablebase evidence recorded: …"* `[V]`, and `perfect_tablebase` serves live Syzygy, measured 33 ms median in the K9 dossier); what remains of the Syzygy half is **reach, not rendering** — the sentences surface only where evidence references resolve, and the A4 packet omission below governs. The LLM half stands: full evidence-bound LLM rendering remains unbuilt. **Corrected 2026-08-21 on A4, and RE-CORRECTED 2026-08-22 on the F1/F2 landing (mirror of `gates.md` B4; claude on the O1 program's landed facts):** the A4-era negative — fourteen shipped producer paths ending in five incompatible delivery states, four renderer-visible, zero joins — **is history, discharged by F1/F2**: the compiled contract now closes at **20 producers / 126 projections / 25 consumers / 175 bindings**, plus 33 operand-preserving semantic events, 33 research eligibility rows and one complete-local-population policy, with exact value factories (evidence value authority, 2026-09-24) and **one rendering authority**. The standing residual is the D717 sentence: F1/F2 established authority and mechanics, **not** completeness or a learner experience — F2 deliberately grants no product-module admission, and Feedback Stage 2 + F5's module eligibility/presets remain |
...
| B8 — platform | desktop shell, responsive/PWA transformation, self-hosted engines/providers, read-only share links, accessibility | deployment shipped incl. the light profile (D5 closed); share links via live platform. Residuals: PWA transformation, settings controls. **Corrected 2026-08-21 against the O13 Choice-C floor (R18, `design/research/release-platform-audit.md`, 2026-08-20): negative.** The provider-off Compose core functions — a rehearsal completes and persists with no cloud secret — but the 1.0 floor is unmet: no account export/backup contract, deletion retains solo runs, Maia loss leaves capabilities green, missing distributed notices/SBOM, PWA manifest-only. **Re-corrected 2026-08-22 (mirror of `gates.md` B8):** ~~pointer-only/incorrect board move entry, Tab traps~~ — **repaired by `accessible-board-input` (`2b68103`)**: one controller, five input modes, a 150-cell permanent matrix and a semantic post-gesture grid; owner device/AT validation remains its D3 discharge. The floor is otherwise **still unmet independently**. F12 owns the floor; below it the build may not be called 1.0-with-known-issues (O13) |
| B9 — structural reading | feature predicates computed, authorable and rendered; denial, outpost, diagonal, pressure and discovered-consequence readables work with no engine; each abstains honestly and attributes judgement rather than asserting it | **shipped 2026-08-14 (`structural-reading`)** — twelve scoped feature predicates, dual readable/authorable role, Pack B graded by structural consequence, closed-by-default disclosure. Rung-0 layer is real. **Qualified 2026-08-21 by A3 (`design/research/detector-semantic-conformance.md`, 2026-08-20):** the matcher/reader machinery ships, but only **11/18** structural families round-trip (seven are subset/lossy/matcher-only), all six transition families are lossy, 0/3,371 transition observations retain squares — and **zero complete families are unconditionally admitted as learner events** under O2's eligibility bar. The machinery landing stands; the learner-event projection does not yet |
| B10 — adaptive guidance | live phase/structure classification in-run; assistance configurable per session context; pivotal moments auto-detected in play with no author; endgame steering names a technique rather than a move | **shipped 2026-08-14 (`adaptive-guidance`)** — attributed phase classification with honest abstention, silent-by-default preferences, passive pivotal markers (two-decision option collapse), disclosure-gated human splits, endgame technique naming grounded on cited, versioned setup conventions (Lucena/Philidor/Vančura named only when `theory.endgame.setup_match@1` matches, as geometry rather than outcome; rebuilt 2026-09-24 on the owner ruling after [[D2495]]; reachability remains [[D2497]]), retrospective eval pivots, packet-bound voice seam with deterministic fallback. **Qualified 2026-08-21 by A5 (`design/research/workflow-default-conformance.md`, 2026-08-20):** the historical feature pieces ship, but the production surface has **one unnamed default**, only **2/6** direct workflow bindings and **no per-kind ceiling** (session-kind permissions are byte-identical) — so the *integrated* adaptive-guidance outcome ("assistance configurable per session context" as opinionated workflow presets/ceilings) is **not shipped in full**. O4 owns the workflow/preset model; exact names, budgets and defaults remain R3's |
| B11 — reusable shapes | a shape entry authored once attaches to every position where its trigger fires; a drill is generated from position source + structural objective + resistance; one play surface serves both | **shipped 2026-08-14 (`shape-library`)** per the split ruling: entries reference-only in packs, derived-projection markers, the position player. 23 entries authored (count corrected 2026-08-14 by the reconciliation delta — it read 24; two commissioned entries, the London wedge and the KID arrangement chain, are outstanding, not authored) |
