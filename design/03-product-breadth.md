...
| B4 — evidence | authored, Stockfish, Maia, corpus/historical, Syzygy, structural/temporal, and LLM-rendered layers work with timing controls | authored ✓, Stockfish ✓, Maia ✓, **corpus/recency ✓ (`runtime-corpus-evidence`)**, structural ✓ (B9), voice seam ✓. True residual **(narrowed 2026-08-21, owner-approved D650)**: ~~Syzygy runtime rendering~~ — tablebase evidence **renders** (`evidence-sentences.ts:163-188` emits *"Exact tablebase evidence recorded: …"* `[V]`, and `perfect_tablebase` serves live Syzygy, measured 33 ms median in the K9 dossier); what remains of the Syzygy half is **reach, not rendering** — the sentences surface only where evidence references resolve, and the A4 packet omission below governs. The LLM half stands: full evidence-bound LLM rendering remains unbuilt. **Corrected 2026-08-21 on A4 (`design/research/evidence-contract-topology.md`, 2026-08-20): the current *integrated* result is negative** — fourteen shipped producer paths end in five incompatible delivery states, only four producers are renderer-visible, production has **zero** module/workflow joins, and `EvidencePacket` omits transition/Maia/Explorer/opening identity. The per-layer landings above stand as history; the joined evidence plane O1's manifest requires does not yet exist (F1/F2 own it) |
