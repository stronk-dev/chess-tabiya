...
| Opponent policy broker (theory_strict…strong_engine modes; policy mixer for coherence) | 📐 · feasibility = exploration **Q5** · DESIGN-GAPS 2026-08-12: (1) pinned Maia UCI ranks MultiPV candidates but does not expose its internally computed move-policy mass; theory_strict proportional filtering needs an adapter extension or amended sampling contract (WDL must not be misused as move probability); (2) living run-schema v0.3's strict `opponent.move_selected` payload has no selection/engine-identity fields even though the accepted selector contract requires them, so writer-side commitment needs a declared schema amendment | `rfc/engine-workers.md` EW-C1/EW-C3, `arch/rfcs/RFC-0003` sketch, `arch/08` |
