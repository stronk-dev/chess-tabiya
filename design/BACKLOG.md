...
| D12 | **Pack C has never been playable — verified by execution.** Its `still-holding` checkpoint triggers on `materialBalance {black, atLeast, -1}`, and Black is *already* a pawn down at the root (White e4/f2/g2/h2 vs Black f7/g7/h6, balance −1), so the condition is true before anyone moves. The opponent's first move fires it, the objective grades `achieved`, `achieved` is absorbing, and `commitMove` throws `RUN_TERMINATED` before the learner ever moves. The one real `hold` pack we authored is a drill you cannot play | 🐞 found 2026-08-12 by the Outcome Drill draft |
| D12a | **No lint catches a trigger that is already true at the root.** Pack C passes `pack-check` green. A condition satisfied by `start.fen` is always an authoring error, and nothing detects it | 🐞 |
| D12b | **`achieved` is absorbing, so grading a hold as achieved ends the drill.** A defensive objective that succeeds *by continuing* cannot use a state that stops play. The proposed law: a state that stops play may only be entered where play has actually ended | 🐞 |
| D12c | **The disclosure barrier opens at ply 1** in the same scenario — `delayed_checkpoint` discloses on the first `checkpoint.reached`, so a checkpoint that fires immediately reveals everything before the learner has made a decision. Anti-contamination defeated by an authoring slip | 🐞 |
| D13 | **Zero-length segments are emitted.** `reachCheckpoint` pairs each checkpoint with the previous one, so two checkpoints firing at the same node produce a segment spanning nothing | 🐞 |
